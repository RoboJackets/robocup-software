...
SSLLog::SSLLog(string filename) {
    ...
    if (strncmp(fileHeader.name, DEFAULT_FILE_HEADER_NAME, sizeof(fileHeader.name)) == 0) {
        cout << "File format version " << fileHeader.version << " detected." << endl;
            ...
            while (!in.eof()) {
                ...
                if (dataHeader.messageType == MESSAGE_SSL_VISION_2014) {
                    ...
                    if (packet.ParseFromArray(data, dataHeader.messageSize)) {
                        visionPackets++;
                        vision2014.push_back(make_shared<SSL_WrapperPacket>(packet));
                        //cout << setiosflags(ios::fixed) << setprecision(3) << (double) dataHeader.timestamp / 1e9
                        //          << ": Read " << visionPackets << " vision packets and " << refereePackets << " referee packets!" << endl;
                    } else {
