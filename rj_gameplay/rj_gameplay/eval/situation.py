...
def get_situation(world_state: stp.rc.WorldState, possession_state: poss_eval.PossessionEvaluator.PropsT, pileup_state: pile_eval.PileupEvaluator.PropsT):
    ...
    if(world_state.game_info.game_state is rc.GameState.HALT):
        return rc.eval.Situation.NO_SITUATION
    
    field_loc = situation_utils.ballLocation(world_state)
    possession = possession_state.possession
    pileup = pileup_state.is_pileup
    gamestate = world_state.game_info


def get_our_possession_situation():
    pass

def get_their_possession_situation():
    pass

def get_pileup_situation(field_loc: situation_utils.FieldLoc) -> rc.eval.Situation:
    return None

def get_scramble_situation(field_loc: situation_utils.FieldLoc):
    pass

def get_restart_situation(field_loc: situation_utils.FieldLoc, gamestate: rc.GameState) -> rc.eval.Situation:
    return None 
    #This one at least makes senes because it's a 
