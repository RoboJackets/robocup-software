...
class MarkerRole(stp.role.Role):
	...
	def tick(self, world_state: stp.rc.WorldState, target_robot: stp.rc.Robot) -> RobotIntent:
		if target_robot is not None:
			self.target_robot = target_robot
		if self.mark_skill is None or target_robot is not None:
			self.mark_skill = mark.Mark(robot=self.robot, target_robot=self.target_robot)
