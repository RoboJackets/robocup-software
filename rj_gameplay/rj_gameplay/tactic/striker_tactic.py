...
def blocker_margin(
    kick_origin: np.array, kick_target: np.array, kick_speed: float, blocker: rc.Robot
):
...
def find_target_point(world_state: rc.WorldState, kick_speed) -> np.ndarray:
    ...
    cost, point = min(
        [
            (kick_cost(point, kick_speed, world_state.ball.pos, world_state), point)
...
class LineKickStrikerTactic(tactic.ITactic):
    ...
    def __init__(
        ...
        self.target_point = target_point
        self.shoot = tactic.SkillEntry(
            line_kick.LineKickSkill(action_client_dict, robot=None, target_point=None)
        )
        self.capture_cost = CaptureCost()
