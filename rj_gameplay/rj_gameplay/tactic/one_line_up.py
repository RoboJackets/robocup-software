...
class OneLineUp(tactic.ITactic):
    ...
    def __init__(self, action_client_dict: Dict[Type[Any], List[Any]], robot_id):
        ...
        self.move_right = tactic.SkillEntry(
            move.Move(
                action_client_dict, target_point=np.array([self.right_x, self.start_y])
            )
        )
        self.move_left = tactic.SkillEntry(
            move.Move(
                action_client_dict, target_point=np.array([self.left_x, self.start_y])
            )
        )
    ...
    def get_requests(
        ...
        move_request = role.RoleRequest(
            role.Priority.HIGH, True, self.cost, self.constraint
        )
        # has_ball = True
    ...
    def is_done(self, world_state):
        return self.move_right.skill.is_done(
            world_state
        ) and self.move_left.skill.is_done(world_state)
