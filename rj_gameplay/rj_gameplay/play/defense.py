...
from rj_gameplay.tactic import goalie_tactic, nmark_tactic, wall_tactic, runner_tactic
...
class Defense(stp.play.Play):
    ...
    def tick(
        ...
        if self._state == State.INIT:
            ...
            num_wallers = min(4, len(world_state.our_visible_robots) - 2)
            if num_wallers > 1:
                self.prioritized_tactics.append(
                    wall_tactic.WallTactic(world_state, num_wallers)
                )
            num_markers = len(world_state.our_visible_robots) - (2 + num_wallers)
            if num_markers > 1:
                self.prioritized_tactics.append(
