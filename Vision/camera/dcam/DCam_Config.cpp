...
DCam_Feature::DCam_Feature(dc1394camera_t *camera, dc1394feature_info_t *info,
...
{
	dc1394feature_mode_t mode = on ? DC1394_FEATURE_MODE_AUTO
...
void DCam_Feature::slider0_changed(int value)
{
	...
	{
		// White balance
		dc1394_feature_whitebalance_set_value(_camera,
		        slider[0]->sliderPosition(), slider[1]->sliderPosition());
	}
	else
	{
		// Everything else
...
DCam_Config::DCam_Config(Camera::DCam *dcam, QWidget *parent) :
	QWidget(parent)
{
	_dcam = dcam;
	...
	for (int i = DC1394_FEATURE_MIN; i <= DC1394_FEATURE_MAX; i++)
	{
		...
		{
			DCam_Feature *obj = new DCam_Feature(_camera, feature, this, grid,
			        row);
			row += obj->rows;
...
{
	for (QDomElement feature_element = element.firstChildElement(); !feature_element.isNull(); feature_element
	        = feature_element.nextSiblingElement())
	{
		...
		dc1394feature_t feature = (dc1394feature_t) feature_element.attribute(
		        "id", "-1").toInt();
		if (feature < 0)
		{
			continue;
		...
		dc1394_feature_set_mode(_camera, feature,
		        auto_active ? DC1394_FEATURE_MODE_AUTO
		                : DC1394_FEATURE_MODE_MANUAL);
...
void DCam_Config::save(QDomElement element)
{
	...
	for (int i = DC1394_FEATURE_MIN; i <= DC1394_FEATURE_MAX; i++)
	{
		...
		if (feature->available)
		{
			...
			QDomElement feature_element =
			        element.ownerDocument().createElement("feature");
...
{
	for (unsigned int i = 0; i < feature_objects.size(); ++i)
	{
		feature_objects[i]->update_value();
	}
}
