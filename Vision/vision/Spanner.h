...
namespace Vision
{
    ...
    public:
        ...
        Geometry::Point2d raw_center;

        // Center in undistorted coordinates
        Geometry::Point2d center;

        // Center to use for track reports.  By default this is the same as center.
        Geometry::Point2d report_center;
        ...
        // Degrees counterclockwise from reference.
        // Set by identification code.
        // Must be on [-180, 180].
        float direction;

        bool direction_valid() const { return direction >= -180 && direction <= 180; }
    };
    ...
    class Spanner: public Processor
    {
    public:
        ...
        std::list<Group *> &groups() { return _groups; }
        const std::list<Group *> &groups() const { return _groups; }

        // Returns a reference to the span list.
        // May only be used in the vision thread or with the mutex locked.
        const std::list<Span *> spans() const { return _all_spans; }
