...
{
	world_point[0] = Geometry2d::Point(0, 0);
    world_point[1] = Geometry2d::Point(1, 0);
    world_point[2] = Geometry2d::Point(0, 1);

	matrix.loadIdentity();
}

Geometry2d::Point Vision::Transform::transform(Geometry2d::Point in)
{
	Vector3f out = matrix * Vector3f(in.x, in.y, 1);
	return Geometry2d::Point(out[0], out[1]);
}
