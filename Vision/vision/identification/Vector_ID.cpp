...
Vector_ID* Vector_ID::load(QDomElement element, Process* process, Color center)
{
	...
	Vector_Pattern pat(0, Geometry2d::Point(0,0), sequences, pairs);
...
void Vector_ID::identify(Group *center_group)
{
    ...
    {
        Geometry2d::Point facing;
        BOOST_FOREACH(const Vector_Pattern::Pair &pair, _pattern.pairs)
        {
            Geometry2d::Point v = _dots[pair.i1].group->center - _dots[pair.i0].group->center;
            v.rotate(Geometry2d::Point(), pair.angle);
            facing += v;
        }
