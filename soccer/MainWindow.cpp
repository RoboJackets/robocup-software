...
void MainWindow::updateViews() {
  ...
  if (liveFrame && liveFrame->debug_layers_size() > _ui.debugLayers->count()) {
    ...
    for (int i = _ui.debugLayers->count(); i < liveFrame->debug_layers_size();
         ++i) {
      const QString name = QString::fromStdString(liveFrame->debug_layers(i));
      bool enabled =
          !std::any_of(defaultHiddenLayers.begin(), defaultHiddenLayers.end(),
  ...
  if (currentFrame) {
    ...
    if (_quaternion_demo && manual >= 0 && currentFrame->radio_rx().size() &&
    ...
    _frameNumberItem->setData(ProtobufTree::Column_Value, Qt::DisplayRole,
                              frameNumber());
    int elapsedMillis =
        (currentFrame->command_time() - _processor->firstLogTime + 500) / 1000;
  ...
  for (const OurRobot *robot : _processor->state()->self) {
    ...
    bool displaying =
        _robotStatusItemMap.find(robot->shell()) != _robotStatusItemMap.end();
    ...
    if (shouldDisplay) {
      ...
      if (rx.has_battery()) {
        if (rx.hardware_version() == RJ2008 ||
          ...
          cerr << "Unknown hardware revision " << rx.hardware_version()
      ...
      bool showstopper = !hasVision || !hasRadio || hasMotorFault ||
                         kickerFault || ballSenseFault || (batteryLevel < 0.25);
      statusWidget->setShowstopper(showstopper);
