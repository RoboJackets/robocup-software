 ...
 public:
  ...
  [[nodiscard]] bool connected() const {
    ...
    if (!have_msg) {
      ...
      RCLCPP_INFO_STREAM_THROTTLE(node_->get_logger(), clk, throttle_ms,
                                  "[ConfigClient] Waiting on ConfigServer...");
    }
    return have_msg;
  }

  void updateGameSettings(GameSettingsMsg msg);
  void updateFieldDimensions(FieldDimensionsMsg msg);
