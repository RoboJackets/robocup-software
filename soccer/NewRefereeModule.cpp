...
void NewRefereeModule::run() {
    ...
    while (_running) {
        ...
        sent_time = RJ::Time(std::chrono::microseconds(packet->wrapper.packet_timestamp()));
        stage_time_left = std::chrono::milliseconds(packet->wrapper.stage_time_left());
        command_counter = packet->wrapper.command_counter();
        command_timestamp = RJ::Time(std::chrono::microseconds(packet->wrapper.command_timestamp()));
