...
void SpaceNavJoystick::update() {
    ...
    _controls.kick = false;
    _controls.chip = false;
    ...
    while (spnav_poll_event(&sev)) {
        if (sev.type == SPNAV_EVENT_MOTION) {
            ...
            const auto DribbleStepTime = RJ::Seconds(0.3);
            if ((sev.motion.y >
                ...
                if ((now - _lastDribbleTime) > DribbleStepTime) {
                    ...
                    _controls.dribblerPower +=
                        0.1 * -sign<int>(sev.motion.y);
                    if (_controls.dribblerPower < 0)
                        _controls.dribblerPower = 0;
                    if (_controls.dribblerPower > 1)
                        _controls.dribblerPower = 1;
                }

                _controls.translation = Geometry2d::Point(0, 0);
                ...
                //  (1,1)
                _controls.translation = Geometry2d::Point(
                ...
                if (_controls.translation.mag() < 0.05) {
                    _controls.translation = Geometry2d::Point(0, 0);
                }

                //  twisting the spacenav corresponds to sev.motion.ry
                _controls.rotation = sev.motion.ry / 350.0;
                if (abs<float>(_controls.rotation) < 0.05) {
                    _controls.rotation = 0;
                }
            }

            _controls.dribble = _controls.dribblerPower > 0.01;
...
void SpaceNavJoystick::reset() {
    ...
    //  reset commands
    _controls = InputDeviceControlValues();
}
