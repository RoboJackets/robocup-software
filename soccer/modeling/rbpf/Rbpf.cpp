...
void Rbpf::resampleParticles(ParticleVector &in, ParticleVector &out, const int k){
	...
	for(int i=0; i<k; i++){intervals[i] = i*oneOverK;}
	// draw a sample from each interval
	for(int i=0; i<k; i++){
		UIntervals[i] = intervals[i] + oneOverK * (rand() / ((double)RAND_MAX + 1));
	}
	// normalize particle weights
	double weightSum = 0, Neff = 0;
	for(int i=0; i<kIn; i++){weightSum += in[i].weight;}
	for(int i=0; i<kIn; i++){
		if(weightSum==0){in[i].weight = oneOverK;
		}else{in[i].weight /= weightSum;}
	...
	if(Neff < NThresh || k!=kIn){
		...
		for(int i=0; i<k; i++){
			int j=0;
			while(cumulativeWeights[j]<UIntervals[i]){j++;}
			resampleIndex[i] = j;
		}
	}else{
		for(int i=0; i<k; i++){resampleIndex[i]=i;}
	}
	// resample
	for(int i=0; i<k; i++){
