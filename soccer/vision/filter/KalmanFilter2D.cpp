...
KalmanFilter2D::KalmanFilter2D(geometry2d::Point initPos,
    ...
    x_k1_k1 << initPos.x(), initVel.x(), initPos.y(), initVel.y();
    x_k_k1 = x_k1_k1;
    x_k_k = x_k1_k1;
    ...
    P_k1_k1 << p, 0, 0, 0, 0, p, 0, 0, 0, 0, p, 0, 0, 0, 0, p;
    P_k_k1 = P_k1_k1;
    P_k_k = P_k1_k1;
    ...
    double dt =
        *VisionFilterConfig::vision_loop_dt;  // TODO: Take config values
    F_k << 1, dt, 0, 0, 0, 1, 0, 0, 0, 0, 1, dt, 0, 0, 0, 1;
...
void KalmanFilter2D::predictWithUpdate(geometry2d::Point observation) {
    z_k << observation.x(), observation.y();
