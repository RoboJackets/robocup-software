...
public:
     ...
     */
    CameraFrame(RJ::Time t_capture, int camera_id, std::vector<CameraBall> camera_balls,
     ...
     * @param msg
     */
    CameraFrame(const DetectionFrameMsg& msg, const rj_geometry::TransformMatrix& world_to_team,
                double team_angle)
        ...
        for (const DetectionRobotMsg& robot_msg : msg.robots_blue) {
            camera_robots_blue.emplace_back(t_capture, robot_msg, world_to_team, team_angle);
        }
        for (const DetectionRobotMsg& robot_msg : msg.robots_yellow) {
            camera_robots_yellow.emplace_back(t_capture, robot_msg, world_to_team, team_angle);
        }
    }
