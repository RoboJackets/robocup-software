...
void BatteryWidget::paintEvent(QPaintEvent *event) {
    ...
    //  how big the positive terminal is relative to the main part of the battery
    float nubRatio = 0.14;
    ...
    QRectF nubBox = battBounds.adjusted(battBounds.width()*(1-nubRatio), battBounds.height()/4, 0, -battBounds.height()/4);
    painter.drawRoundedRect(nubBox, 0.5, 0.5);


    //  12.5%, 37.5%, 62.5%, 87.5% are the midpoints of the 4 bars
    //  we draw the number of bars corresponding to the point that this battery level is closest to
    ...
    for (int i = 0; i < barCount; i++) {
        painter.drawRect(QRectF(mainBox.left() + pad + (barSpacing+barWidth)*i, mainBox.top() + pad, barWidth, mainBox.height() - pad*2));
    }
}
