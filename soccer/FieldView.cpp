...
void FieldView::paintEvent(QPaintEvent* e)
{
	...
	p.rotate(_rotate * 90);
	p.scale(1.0 / Field_Dimensions.Floor_Length, 1.0 / Field_Dimensions.Floor_Width);
	...
	_screenToWorld *= Geometry2d::TransformMatrix::scale(Field_Dimensions.Floor_Length, Field_Dimensions.Floor_Width);
	...
	_worldToTeam *= Geometry2d::TransformMatrix::translate(0, Field_Dimensions.Field_Length / 2.0f);
	...
	_teamToWorld *= Geometry2d::TransformMatrix::translate(0, -Field_Dimensions.Field_Length / 2.0f);
	...
	p.translate(0, -Field_Dimensions.Field_Length / 2.0f);
...
void FieldView::drawTeamSpace(QPainter& p)
{
	...
	{
		const float FX = Field_Dimensions.Floor_Width / 2;
		const float FY1 = -Field_Dimensions.Field_Border;
		const float FY2 = Field_Dimensions.Field_Length / 2;
		p.fillRect(QRectF(QPointF(-FX, FY1), QPointF(FX, FY2)), QColor(0, 0, 0, 128));
	}
	if (!frame->use_opponent_half())
	{
		const float FX = Field_Dimensions.Floor_Width / 2;
		const float FY1 = Field_Dimensions.Field_Length / 2;
		const float FY2 = Field_Dimensions.Field_Length + Field_Dimensions.Field_Border;
		p.fillRect(QRectF(QPointF(-FX, FY1), QPointF(FX, FY2)), QColor(0, 0, 0, 128));
	}
...
void FieldView::drawField(QPainter& p, const LogFrame *frame)
{
	...
	//reset to center
	p.translate(-Field_Dimensions.Floor_Length/2.0, -Field_Dimensions.Floor_Width/2.0);
	
	p.translate(Field_Dimensions.Field_Border, Field_Dimensions.Field_Border);
	
	p.setPen(QPen(Qt::white, Field_Dimensions.Field_LineWidth*2.0));	//	double-width pen for visibility (although its less accurate)
	p.setBrush(Qt::NoBrush);
	p.drawRect(QRectF(0, 0, Field_Dimensions.Field_Length, Field_Dimensions.Field_Width));
	
	//set brush alpha to 0
	p.setBrush(QColor(0,130,0, 0));
	
	//reset to center
	p.translate(Field_Dimensions.Field_Length/2.0, Field_Dimensions.Field_Width/2.0);
	
	//centerline
	p.drawLine(QLineF(0, Field_Dimensions.Field_Width/2,0, -Field_Dimensions.Field_Width/2.0));
	
	//center circle
	p.drawEllipse(QRectF(-Field_Dimensions.Field_CenterRadius, -Field_Dimensions.Field_CenterRadius,
		Field_Dimensions.Field_CenterDiameter, Field_Dimensions.Field_CenterDiameter));
	
	p.translate(-Field_Dimensions.Field_Length/2.0, 0);
	
	//goal areas
	p.drawArc(QRectF(-Field_Dimensions.Field_ArcRadius, -Field_Dimensions.Field_ArcRadius + Field_Dimensions.Field_GoalFlat/2.f, 2.f * Field_Dimensions.Field_ArcRadius, 2.f * Field_Dimensions.Field_ArcRadius), -90*16, 90*16);
	p.drawArc(QRectF(-Field_Dimensions.Field_ArcRadius, -Field_Dimensions.Field_ArcRadius - Field_Dimensions.Field_GoalFlat/2.f, 2.f * Field_Dimensions.Field_ArcRadius, 2.f * Field_Dimensions.Field_ArcRadius), 90*16, -90*16);
	p.drawLine(QLineF(Field_Dimensions.Field_ArcRadius, -Field_Dimensions.Field_GoalFlat/2.f, Field_Dimensions.Field_ArcRadius, Field_Dimensions.Field_GoalFlat/2.f));
	// Penalty Mark
	p.drawEllipse(QRectF(-Field_Dimensions.Field_PenaltyDiam/2.0f + Field_Dimensions.Field_PenaltyDist, -Field_Dimensions.Field_PenaltyDiam/2.0f, Field_Dimensions.Field_PenaltyDiam, Field_Dimensions.Field_PenaltyDiam));
	
	p.translate(Field_Dimensions.Field_Length, 0);
	...
	// goals
	float x[2] = {0, Field_Dimensions.Field_GoalDepth};
	float y[2] = {Field_Dimensions.Field_GoalWidth/2.0f, -Field_Dimensions.Field_GoalWidth/2.0f};
	
	bool flip = frame->blue_team() ^ frame->defend_plus_x();
	
	QColor goalColor = flip ? Qt::yellow : Qt::blue;
	p.setPen(QPen(goalColor, Field_Dimensions.Field_LineWidth * 2.0));	//	double-width for visibility, not real-life accuracy
	p.drawLine(QLineF(x[0], y[0], x[1], y[0]));
	p.drawLine(QLineF(x[0], y[1], x[1], y[1]));
	p.drawLine(QLineF(x[1], y[1], x[1], y[0]));
	
	x[0] -= Field_Dimensions.Field_Length;
	x[1] -= Field_Dimensions.Field_Length + 2 * Field_Dimensions.Field_GoalDepth;
	
	goalColor = flip ? Qt::blue : Qt::yellow;
	p.setPen(QPen(goalColor, Field_Dimensions.Field_LineWidth * 2.0));
	p.drawLine(QLineF(x[0], y[0], x[1], y[0]));
	p.drawLine(QLineF(x[0], y[1], x[1], y[1]));
	p.drawLine(QLineF(x[1], y[1], x[1], y[0]));
...
void FieldView::resizeEvent(QResizeEvent* e)
{
	...
	if (_rotate & 1)
	{
		needH = roundf(givenW * Field_Dimensions.Floor_Length / Field_Dimensions.Floor_Width);
		needW = roundf(givenH * Field_Dimensions.Floor_Width / Field_Dimensions.Floor_Length);
	} else {
		needH = roundf(givenW * Field_Dimensions.Floor_Width / Field_Dimensions.Floor_Length);
		needW = roundf(givenH * Field_Dimensions.Floor_Length / Field_Dimensions.Floor_Width);
	}
