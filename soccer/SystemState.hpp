...
public:
    ...
    /// since the play needs
    /// to see that it is no longer visible.  We don't want multiple Robots for
    /// the same shell because
    /// that would give the appearance that a new robot appeared when it was
    /// actually just pushed back on
    /// the field.
