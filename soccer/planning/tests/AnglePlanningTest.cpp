...
// (For the case where constraints are not saturated) check that we follow the angle function nearly exactly.
void check_angle_planning_near_exact(const Trajectory& trajectory, const AngleFunction& angle_function, double epsilon = 1e-2) {
    for (auto cursor = trajectory.cursor_begin(); cursor.has_value(); cursor.advance(RJ::Seconds(0.01))) {
        RobotInstant instant = cursor.value();
        EXPECT_NEAR(instant.heading(), angle_function(instant.linear_motion(), instant.heading(), nullptr), epsilon);
    }
}
...
// Make a RobotInstant with the given linear motion and exactly coinciding with the correct angle function.
static RobotInstant make_initial_instant(const LinearMotionInstant& linear,
    ...
    return RobotInstant{Pose{linear.position, angle}, Twist{linear.velocity, angular_velocity}, time};
}

// Dump the angle trajectory to a file. For debugging purposes.
[[maybe_unused]]
static void dump_trajectory(const std::string& file_name, const Trajectory& trajectory, const AngleFunction& angle_fn) {
...
TEST(AnglePlanning, obeys_angle_function_target) {
    ...
    // Create a robot instant. This should allow us to have perfect tracking, because we will start perfectly in line with the angle function.
    RobotInstant initial_instant = make_initial_instant(trajectory.first().linear_motion(), trajectory.first().stamp, angle_fn);
...
TEST(AnglePlanning, DISABLED_start_from_incorrect_angle) {
    ...
    // This time we will use a robot instant with zero heading/angular velocity, which doesn't match up with the profile. This means that we need to catch up.
    ...
    check_angle_planning_near_exact(trajectory.subTrajectory(trajectory.begin_time() + RJ::Seconds(2.0), trajectory.end_time()), angle_fn, 5e-2);
}
