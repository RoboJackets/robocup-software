...
BezierPath::BezierPath(const std::vector<Point>& points, Point vi, Point vf,
    ...
    for (int i = 0; i < num_curves; i++) {
        double time_before = Trapezoid::time_remaining(
            Trapezoid::State{path_length_so_far, start_speed},
            Trapezoid::State{total_path_length, end_speed}, motion_constraints.max_speed,
            motion_constraints.max_acceleration);
        path_length_so_far += (points[i + 1] - points[i]).mag();
        double time_after = Trapezoid::time_remaining(
            Trapezoid::State{path_length_so_far, start_speed},
            Trapezoid::State{total_path_length, end_speed}, motion_constraints.max_speed,
            motion_constraints.max_acceleration);
