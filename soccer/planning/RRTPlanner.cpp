...
bool RRTPlanner::shouldReplan(MotionInstant start, MotionInstant goal,
  ...
  if (SingleRobotPathPlanner::shouldReplan(start, motionConstraints, obstacles,
                                           prevPath)) {
    return true;
  }
...
std::unique_ptr<Path>
RRTPlanner::run(MotionInstant start, const std::unique_ptr<MotionCommand> &cmd,
                const MotionConstraints &motionConstraints,
                const Geometry2d::ShapeSet *obstacles,
  ...
  Planning::PathTargetCommand target =
      *static_cast<Planning::PathTargetCommand *>(cmd.get());
  ...
  if (start.pos == goal.pos) {
    ...
    path->waypoints.emplace_back(MotionInstant(start.pos, Geometry2d::Point()),
                                 0);
...
InterpolatedPath *
RRTPlanner::optimize(InterpolatedPath &path,
                     const Geometry2d::ShapeSet *obstacles,
                     const MotionConstraints &motionConstraints,
...
InterpolatedPath *
RRTPlanner::cubicBezier(InterpolatedPath &path,
                        const Geometry2d::ShapeSet *obstacles,
                        const MotionConstraints &motionConstraints,
                        Geometry2d::Point vi, Geometry2d::Point vf) {
  ...
  for (int i = 0; i < curvesNum; i++) {
    ks[i] =
        1.0 / (getTime(path, i + 1, motionConstraints, startSpeed, endSpeed) -
  ...
  for (int i = 0; i < curvesNum; i++) {
    ...
    for (int j = 0; j < interpolations; j++) {
      ...
      Geometry2d::Point pos = pow(1.0 - t, 3) * p0 +
                              3.0 * pow(1.0 - t, 2) * t * p1 +
