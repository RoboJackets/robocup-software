...
Point TargetVelPathPlanner::calculateNonblockedPathEndpoint(
  ...
  const auto scaledDistRange = boost::irange((int)(minDist * rangeScaleFactor),
                                             (int)(maxDist * rangeScaleFactor));
...
bool TargetVelPathPlanner::shouldReplan(
  ...
  if (!trapezoidalPath) {
    throw std::runtime_error("TargetVelPathPlanner expected a prevPath of type "
                             "'TrapezoidalPath'");
  }
  const float velChange = command.worldVel.mag() - trapezoidalPath->maxSpeed();
...
std::unique_ptr<Path> TargetVelPathPlanner::run(
  ...
  if (shouldReplan(startInstant, cmd, motionConstraints, obstacles,
    ...
    auto path = std::unique_ptr<Path>(
        new TrapezoidalPath(startInstant.pos, startInstant.vel.mag(), endpoint,
                            0, moddedConstraints));
