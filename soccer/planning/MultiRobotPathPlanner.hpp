...
struct PlanRequest {
  PlanRequest(MotionInstant start, std::unique_ptr<MotionCommand> command,
      ...
      : start(start), motionCommand(std::move(command)),
        constraints(constraints), prevPath(std::move(prevPath)),
        obstacles(obs) {}
...
public:
  virtual std::map<int, std::unique_ptr<Path>>
  run(std::map<int, PlanRequest> requests) = 0;
};
