...
// the path at fixed time intervals form t = 0 to t = duration.
    ...
    std::unique_ptr<ConstPathIterator> Path::iterator(RJ::Time startTime, float deltaT) const {
        return std::move(std::make_unique<ConstPathIterator>(this, startTime, deltaT));
    }

    bool Path::pathsIntersect(const std::vector<const Path *> &paths, float *hitTime,
                              Geometry2d::Point *hitLocation, RJ::Time startTime) const {
        ...
        std::transform (std::begin(paths), std::end(paths), std::begin(pathIterators),
