...
void CollectPlanner::createConfiguration(Configuration* cfg) {
    _bufferDistBeforeContact =
        new ConfigDouble(cfg, "Capture/Collect/bufferDistBeforeContact");
    _bufferDistAfterContact =
        new ConfigDouble(cfg, "Capture/Collect/bufferDistAfterContact");
    _ballSpeedApproachDirectionCutoff = new ConfigDouble(
        cfg, "Capture/Collect/ballSpeedApproachDirectionCutoff");
    _touchDeltaSpeed = new ConfigDouble(cfg, "Capture/Collect/touchDeltaSpeed");
    _targetChangeThreshold =
        new ConfigDouble(cfg, "Capture/Collect/targetChangeThreshold");
    _controlAccelScalePercent =
        new ConfigDouble(cfg, "Capture/Collect/controlAccelScalePercent");
...
Trajectory CollectPlanner::course(PlanRequest&& request) {
    ...
    if (!targetInstant || targetInstant->pose.position().distTo(
        ...
        targetInstant = RobotInstant{Pose{noisyTargetPoint, 0},
                                     Twist{noisyTargetVel, 0}, RJ::Time{0s}};
    }
...
Trajectory CollectPlanner::control(PlanRequest&& request) {
    ...
    double accel =
        request.constraints.mot.maxAcceleration * *_controlAccelScalePercent;
    double stoppingDist =
        std::pow(startInstant.velocity.linear().mag(), 2) / (2 * accel);
    Point targetPoint =
        startInstant.pose.position() + stoppingDist * approachDir;
    RobotInstant targetInstant{Pose{targetPoint, 0}, {}, RJ::Time{0s}};
    request.motionCommand = PathTargetCommand{targetInstant};
    request.constraints.mot.maxSpeed = std::min(
        startInstant.velocity.linear().mag(), request.constraints.mot.maxSpeed);
...
void CollectPlanner::processStateTransitions(const PlanRequest& request) {
    ...
    bool fineToCourse =
        collectState == CollectState::Fine &&
        distToBall > distCutoffToFine + distToBallAtContact + 0.2;
    bool controlToCourse =
        collectState == CollectState::Control &&
        distToBall > *_distCutoffToControl + distToBallAtContact + 0.072;
