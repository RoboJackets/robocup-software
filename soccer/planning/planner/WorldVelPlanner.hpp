...
namespace Planning {
    ...
    public:
        ...
        Trajectory plan(PlanRequest&& request) {
            ...
            Point targetVel = std::get<WorldVelCommand>(request.motionCommand).worldVel;
            Point targetPoint = request.start.pose.position() + targetVel.normalized(.2);
            request.constraints.mot.maxSpeed = targetVel.mag();
            RobotInstant goalInstant{Pose{targetPoint, 0}, Twist{targetVel, 0}, RJ::Time{0}};
