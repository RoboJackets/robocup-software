...
protected:
    ...
    public:
        AngleFunctionPath(std::unique_ptr<Path> path = nullptr, boost::optional<std::function<AngleInstant(MotionInstant)>> angleFunction = boost::none) : path(std::move(path)), angleFunction(angleFunction) {}
        ...
        virtual std::unique_ptr<Path> subPath(
            ...
            return std::make_unique<AngleFunctionPath>(path->subPath(startTime, endTime), angleFunction);
        }
        ...
        virtual std::unique_ptr<Path> clone() const override {
            return std::make_unique<AngleFunctionPath>(path->clone(), angleFunction);
        }
