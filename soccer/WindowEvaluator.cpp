...
void WindowEvaluator::obstacle_range(vector<Window>& windows, double& t0, double& t1) {
...
void WindowEvaluator::obstacle_robot(vector<Window>& windows, Point origin, Segment target, Point bot_pos) {
  ...
  for(int i = 0; i < 2; i++) {
    ...
    Point intersect;
    if(edge.intersects(target, &intersect) && (intersect - origin).dot(edge.delta()) > d) {
...
WindowingResult WindowEvaluator::eval_pt_to_seg(Point origin, Segment target) {
  ...
  bot_locations.insert(bot_locations.end(), hypothetical_robot_locations.begin(), hypothetical_robot_locations.end());
