...
VisionReceiver::VisionReceiver()
  ...
  raw_packet_pub_ = create_publisher<RawProtobufMsg>("vision/raw_protobuf", 10);
  vision_packet_pub_ =
...
void VisionReceiver::setPort(int port) {
  ...
  if (static_cast<bool>(bind_error)) {
    std::cerr << "Vision port bind failed with error: " << bind_error.message()
              << std::endl;
    return;
  }
...
bool VisionReceiver::shouldRemove(bool defendPlusX, double x) {
  ...
  return (!use_their_half &&
         ...
         (!use_our_half && ((defendPlusX && x > 0) || (!defendPlusX && x < 0)));
}
