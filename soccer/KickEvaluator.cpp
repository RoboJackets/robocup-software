...
void KickEvaluator::createConfiguration(Configuration* cfg) {
    kick_std_dev = std::make_unique<ConfigDouble>(cfg, "KickEvaluator/kick_std_dev", 0.04);
    kick_mean = std::make_unique<ConfigDouble>(cfg, "KickEvaluator/kick_mean", 0);
    robot_std_dev = std::make_unique<ConfigDouble>(cfg, "KickEvaluator/robot_std_dev", 0.3);
    start_x_offset = std::make_unique<ConfigDouble>(cfg, "KickEvaluator/start_x_offset", 0.1);
}

KickEvaluator::KickEvaluator(SystemState* systemState) : system(systemState) {}

KickResults KickEvaluator::eval_pt_to_pt(Point origin, Point target, float targetWidth) {
    Point dir = (target - origin).perpCCW().normalized();
    Segment seg = Segment{target + dir * (targetWidth / 2), target - dir * (targetWidth / 2)};
...
KickResults KickEvaluator::eval_pt_to_seg(Point origin, Segment target) {
    ...
    // Polar bot locations
    // <Dist, Angle>
    vector<tuple<float, float> > botLocations = convert_robots_to_polar(origin, center);
    ...
    for (tuple<float, float>& loc : botLocations) {
        ...
        // Robot Past Target
        distPastTarget = static_cast<float>(get<0>(loc) - (origin - center).mag());
        ...
        if (distPastTarget > 0 && fabs(get<1>(loc)) < M_PI / 2) {
            ...
            botVertScales.push_back(1 - erf(distPastTarget / (*robot_std_dev * sqrt(2))));
        } else {
    ...
    function<tuple<float, float>(float)> keFunc =
        bind(&eval_calculation, std::placeholders::_1, (kick_mean->value()), (kick_std_dev->value()), cref(botMeans),
             cref(botStDevs), cref(botVertScales), targetWidth / -2, targetWidth / 2);
    ...
    KickEvaluator::init_gradient_configs(parallelConfig, keFunc, botMeans, botStDevs, targetWidth / -2, targetWidth / 2);
...
tuple<float, float> KickEvaluator::eval_calculation(const float x, const float kmean, const float kstdev,
    ...
    sterm = sqrtpi_2 * (sqrt1_kstdev2 - kstdev * erf((kx + bLeft) / sqrt2_kstdev));

    tterm = sqrtpi_2 * (sqrt1_kstdev2 - kstdev * erf((kx + bRight) / sqrt2_kstdev));
...
float KickEvaluator::get_target_angle(const Point origin, const Segment target) {
    ...
    return static_cast<float>(abs(left.angleBetween(right)));
}
...
tuple<float, float> KickEvaluator::rect_to_polar(const Point origin, const Point target, const Point obstacle) {
    Point obstacleDir = obstacle - origin;
    Point targetDir = target - origin;

    return make_tuple(obstacleDir.mag(), fixAngleRadians(targetDir.angleBetween(obstacleDir)));
}

vector<tuple<float, float> > KickEvaluator::convert_robots_to_polar(const Point origin, const Point target) {
    ...
    transform(hypothetical_robot_locations.begin(), hypothetical_robot_locations.end(), back_inserter(botLocations),
              [target, origin, this](Point obstacle) { return rect_to_polar(origin, target, obstacle); });

    return botLocations;
}

void KickEvaluator::init_gradient_configs(ParallelGradient1DConfig& pConfig, function<tuple<float, float>(float)>& func,
                                          const vector<float>& robotMeans, const vector<float>& robotStDevs,
                                          const float boundaryLower, const float boundaryUpper) {
    pConfig.GA1DConfig.reserve(robotStDevs.size());

    // Standard Gradient Configs
    const float dxError = 0.05;
    float maxXMovement = *min_element(robotStDevs.begin(), robotStDevs.end()) * 2;
    ...
    // Add left boundary
    auto startX = static_cast<float>(boundaryLower + *start_x_offset * *kick_std_dev);
    ...
    for (int i = 0; i < robotMeans.size(); i++) {
        ...
        for (int side = -1; side <= 1; side += 2) {
            startX = robotMeans.at(i) + side * *start_x_offset * robotStDevs.at(i);
    ...
    for (tuple<float, float> xStart : xStarts) {
        pConfig.GA1DConfig.emplace_back(&func, get<1>(xStart), get<0>(xStart), dxError, maxXMovement,
                                        temperatureDescent, temperatureMin, maxIterations, maxValue, maxThresh);
    }

    pConfig.xCombineThresh =
        static_cast<float>(*min_element(robotStDevs.begin(), robotStDevs.end()) * *start_x_offset / 2);
}