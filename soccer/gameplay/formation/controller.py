...
class Controller(composite_behavior.CompositeBehavior):
    ...
    def __init__(self):
        ...
        self.Goalie = positions.simple_goalie.SimpleGoalie(goalie, "Goalie")

        # List of all positions on the field
        # Sorted from left to right, front to back
        self.positions = [
            self.Striker_Left, self.Striker_Right, self.Midfielder_Center,
            self.Defender_Left, self.Defender_Right, self.Goalie
        ]
        ...
        self.Striker_Left.relative_pos = robocup.Point(-1,
                                                       Controller.STRIKER_Y)
        self.Striker_Right.relative_pos = robocup.Point(1,
                                                        Controller.STRIKER_Y)

        self.Midfielder_Center.relative_pos = robocup.Point(
            0, Controller.MIDFIELDER_Y)

        self.Defender_Left.relative_pos = robocup.Point(-0.5,
                                                        Controller.DEFENDER_Y)
        self.Defender_Right.relative_pos = robocup.Point(0.5,
                                                         Controller.DEFENDER_Y)
        ...
        self.Striker_Left.pass_options = [
            self.Striker_Right, self.Midfielder_Center, self.Defender_Left
        ]
        self.Striker_Right.pass_options = [
            self.Striker_Left, self.Midfielder_Center, self.Defender_Right
        ]

        self.Midfielder_Center.pass_options = [
            self.Striker_Left, self.Striker_Right, self.Defender_Left,
            self.Defender_Right
        ]

        self.Defender_Left.pass_options = [
            self.Striker_Left, self.Midfielder_Center, self.Defender_Right
        ]
        self.Defender_Right.pass_options = [
            self.Striker_Right, self.Midfielder_Center, self.Defender_Left
        ]
    ...
    def update_formation_center(self):
        ...
        possible_lines = [
            Controller.STRIKER_Y, Controller.MIDFIELDER_Y,
            Controller.DEFENDER_Y
        ]
        ...
        for l in possible_lines:
            real_target = self.formation_center.y + (
                l + Controller.CENTER_OFFSET) * self.formation_length / 2
        ...
        self.formation_center.y = main.ball().pos.y - (
            target_line + Controller.CENTER_OFFSET) * self.formation_length / 2
        self.formation_center.x = alpha * self.formation_center.x + (
            1 - alpha) * main.ball().pos.x
