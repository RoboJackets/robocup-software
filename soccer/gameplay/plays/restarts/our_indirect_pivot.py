...
class OurIndirectPivot(standard_play.StandardPlay):
    ...
    def __init__(self, indirect=None):
        ...
        self.add_transition(OurIndirectPivot.State.passing,
                            ...
                            lambda: self.pass_bhvr.is_done_running(), 'Pass Completed')
    ...
    def score(cls):
        ...
        if OurIndirectPivot.Running or (gs.is_ready_state() and (gs.is_our_direct() or gs.is_our_indirect()) and main.ball().pos.y < (
    ...
    def on_enter_passing(self):
        ...
        if abs(main.ball().pos.x)>2:
            threat_point = robocup.Point(-2*main.ball().pos.x/main.ball().pos.x,main.ball().pos.y)
        elif abs(main.ball().pos.x)<.25:
            threat_point = robocup.Point(-(.5+main.ball().pos.x),main.ball().pos.y)
        else:
