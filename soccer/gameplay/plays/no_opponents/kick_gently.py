...
class KickGently(play.Play):
    ...
    def __init__(self, indirect=None):
        ...
        self.add_transition(
            behavior.Behavior.State.start, KickGently.State.passing,
            lambda: True, 'immediately')

        self.add_transition(
            KickGently.State.passing, KickGently.State.dumb,
            lambda: abs(main.ball().pos.x) > KickGently.Threshold and self.subbehavior_with_name('passer').state==behavior.Behavior.State.completed, 'not centered')

        self.add_transition(
            KickGently.State.dumb, KickGently.State.passing,
            lambda: True, 'immediately')

        self.add_transition(
            KickGently.State.passing, KickGently.State.kicking,
            lambda: abs(main.ball().pos.x) <= KickGently.Threshold,'ball is centered')

        self.add_transition(
            KickGently.State.kicking, KickGently.State.kicking,
            lambda: abs(main.ball().pos.x) <= KickGently.Threshold and self.subbehavior_with_name('kicker').state==behavior.Behavior.State.completed,'ball is centered')

        self.add_transition(
            KickGently.State.kicking, KickGently.State.passing,
            lambda: abs(main.ball().pos.x) > KickGently.Threshold,'ball is not centered')
    ...
    def on_enter_kicking(self):
        ...
        kicker.aim_params={'error_threshold':.05,'desperate_timeout': float("inf"),'max_steady_ang_vel':4}
