...
boost::python::tuple
WinEval_eval_pt_to_opp_goal(WindowEvaluator *self,
                            const Geometry2d::Point *origin) {
  if (origin == nullptr)
    throw NullArgumentException{"origin"};
  boost::python::list lst;
...
boost::python::tuple
WinEval_eval_pt_to_our_goal(WindowEvaluator *self,
                            const Geometry2d::Point *origin) {
  if (origin == nullptr)
    throw NullArgumentException{"origin"};
  boost::python::list lst;
...
BOOST_PYTHON_MODULE(robocup) {
  ...
  class_<Geometry2d::CompositeShape, bases<Geometry2d::Shape>>("CompositeShape",
                                                               init<>())
