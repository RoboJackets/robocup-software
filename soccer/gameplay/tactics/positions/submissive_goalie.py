...
class SubmissiveGoalie(single_robot_composite_behavior.SingleRobotCompositeBehavior):
    ...
    def __init__(self):
        ...
        self.add_transition(behavior.Behavior.State.start,
            SubmissiveGoalie.State.chill,
            lambda: True,
            "immediately")

        self.add_transition(SubmissiveGoalie.State.chill,
            SubmissiveGoalie.State.defend,
            lambda: main.ball().valid,
            "ball is valid")

        non_chill_states = [s for s in SubmissiveGoalie.State if s != SubmissiveGoalie.State.chill]
        ...
        for state in non_chill_states:
            self.add_transition(state,
                SubmissiveGoalie.State.setup_penalty,
                lambda: main.game_state().is_their_penalty() and
                        main.game_state().is_setup(),
                "setting up for opponent penalty")

        for state in [s2 for s2 in non_chill_states if s2 != SubmissiveGoalie.State.intercept]:
            self.add_transition(state,
        ...
        for state in [s2 for s2 in non_chill_states if s2 != SubmissiveGoalie.State.clear]:
            self.add_transition(state,
        ...
        for state in [s2 for s2 in non_chill_states if s2 != SubmissiveGoalie.State.defend]:
            self.add_transition(state,
                SubmissiveGoalie.State.defend,
        ...
        for state in [s2 for s2 in non_chill_states if s2 != SubmissiveGoalie.State.block]:
            self.add_transition(state,
    ...
    def execute_setup_penalty(self):
        ...
        dest = shot_line.intersection(SubmissiveGoalie.RobotSegment)
        if dest == None:
            self.robot.move_to(robocup.Point(0, constants.Robot.Radius))
        else:
            dest.x = max(-SubmissiveGoalie.MaxX + constants.Robot.Radius, dest.x)
            dest.y = min(SubmissiveGoalie.MaxX - constants.Robot.Radius, dest.x)
    ...
    def execute_block(self):
        ...
        if opposing_kicker is not None:
            ...
            if best is not None:
                ...
                dest.x = min(SubmissiveGoalie.MaxX, dest.x)
                dest.x = max(-SubmissiveGoalie.MaxX, dest.x)
                self.robot.move_to(dest)
                return
        self.robot.move_to(robocup.Point(0,constants.Robot.Radius))
    ...
    def execute_defend(self):
        dest_x = main.ball().pos.x / constants.Field.Width * SubmissiveGoalie.MaxX
