...
class WingDefender(single_robot_behavior.SingleRobotBehavior):
    ...
    def calc_move_point(self):
        if self._mark_pos != None:
            goal_line, shot_pt = evaluation.defensive_positioning.goalside_mark_segment(
                self._mark_pos,
                self.robot,
                ball=False,
                kick_eval=self.kick_eval)
            ballside_line = evaluation.defensive_positioning.ballside_mark_segment(
                self._mark_pos, main.ball().pos)
            total_angle = (main.ball().pos - self._mark_pos
                           ).angle_between(shot_pt - self._mark_pos)
            goal_angle = (main.ball().pos - self._mark_pos).normalized().dot(
                (shot_pt - self._mark_pos).normalized())
            ...
            if goal_angle < -.9:
                #print("Striker")
                if (self._mark_pos - main.ball().pos).mag() > (
                        constants.TheirChipping[1] + constants.Robot.Radius * 2
                ):
                    #print("Far Away")
            ...
            norm_goal_line = ((shot_pt - self._mark_pos).normalized() *
                              self.distance + self._mark_pos)
            angle = -1 * angle if main.ball(
            ).pos.x - self._mark_pos.x < 0 else angle
