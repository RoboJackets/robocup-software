...
class Placement(single_robot_composite_behavior.SingleRobotCompositeBehavior):
    ...
    def __init__(self):
        ...
        self.add_transition(behavior.Behavior.State.start,
            Placement.State.capturing, lambda: True, 'immediately')

        self.add_transition(Placement.State.capturing, Placement.State.placing,
            lambda: self.subbehavior_with_name('capture').state==behavior.Behavior.State.completed,
            'done capturing')

        self.add_transition(Placement.State.placing, Placement.State.standby,
