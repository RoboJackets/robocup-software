...
class TestPassing(unittest.TestCase):
	...
	def test_eval_pass(self):
		...
		self.assertEqual(run_function(0, 0, 0, 0), fail, "point pass to itself fail")
		...
		self.assertEqual(run_function(0, 0, 0, length / 2, [bot1]), success, "fail excluded_robots")

		# Robot halfway between shooter and goal point. 
		self.set_bot_pos(bot1, 0, length / 4)
		self.assertGreater(run_function(0, 0, 0, length / 2), fail, "robot halfway between shooter and goal point considered complete failure")
		self.assertLess(run_function(0, 0, 0, length / 2), success, "robot halfway between shooter and goal point considered complete success")
