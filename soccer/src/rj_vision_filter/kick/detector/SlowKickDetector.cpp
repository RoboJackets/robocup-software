...
bool SlowKickDetector::addRecord(RJ::Time calcTime, const WorldBall& ball,
    ...
    if (stateHistory.size() > static_cast<size_t>(kick::detector::PARAM_slow_kick_hist_length)) {
...
bool SlowKickDetector::detectKick(KickEvent* kickEvent) {
    ...
    for (int i = 0; i < stateHistory.at(0).yellowRobots.size(); i++) {
        bool allValid = std::all_of(stateHistory.begin(), stateHistory.end(), [i](VisionState& v) {
    ...
    for (int i = 0; i < stateHistory.at(0).blueRobots.size(); i++) {
        ...
        // Just take this and return a kick event
        if (checkAllValidators(robotList, ballList)) {
            *kickEvent = KickEvent(stateHistory.at(0).calcTime, stateHistory.at(0).blueRobots.at(i),
                                   stateHistory);
...
bool SlowKickDetector::checkAllValidators(const std::vector<WorldRobot>& robot,
    ...
    return distanceValidator(robot, ball) && velocityValidator(robot, ball) &&
           distanceIncreasingValidator(robot, ball) && inFrontValidator(robot, ball);
}
...
bool SlowKickDetector::velocityValidator(const std::vector<WorldRobot>& /*robot*/,
                                         const std::vector<WorldBall>& ball) {
    ...
    for (int i = 0; i < ball.size() - 1; i++) {
        vel.at(i) = (ball.at(i + 1).getPos() - ball.at(i).getPos()).mag() / PARAM_vision_loop_dt;
    }
...
bool SlowKickDetector::distanceIncreasingValidator(const std::vector<WorldRobot>& robot,
                                                   const std::vector<WorldBall>& ball) {
...
bool SlowKickDetector::inFrontValidator(const std::vector<WorldRobot>& robot,
    ...
    for (int i = 0; i < robot.size(); i++) {
        Geometry2d::Point normal =
            Geometry2d::Point(cos(robot.at(i).getTheta()), sin(robot.at(i).getTheta()));
