...
void World::update_with_camera_frame(RJ::Time calc_time, const std::vector<CameraFrame>& new_frames,
    ...
    if (update_all) {
        for (unsigned long i = 0; i < cameras_.size(); i++) {
...
void World::update_world_objects(RJ::Time calc_time) {
    ...
    for (Camera& camera : cameras_) {
        if (camera.get_is_valid()) {
            ...
            for (unsigned long i = 0; i < camera_robots_yellow.size(); i++) {
            ...
            // Take the best kalman filter from the camera
            for (unsigned long i = 0; i < camera_robots_blue.size(); i++) {
    ...
    for (unsigned long i = 0; i < robots_yellow_.size(); i++) {
    ...
    for (unsigned long i = 0; i < robots_blue_.size(); i++) {
