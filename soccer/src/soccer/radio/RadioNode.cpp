...
RadioNode::RadioNode(Context* context, bool simulation, bool blue_team)
    : _context(context) {
    _lastRadioRxTime = RJ::Time(std::chrono::microseconds(RJ::timestamp()));
    _simulation = simulation;
    _was_blue_team = blue_team;
    _radio =
        _simulation
            ? static_cast<Radio*>(new SimRadio(_context, blue_team))
...
void RadioNode::switchTeam(bool blue_team) { _radio->switchTeam(blue_team); }
