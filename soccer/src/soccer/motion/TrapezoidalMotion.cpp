...
double Trapezoidal::getTime(double distance, double path_length,
                            double max_speed, double max_acc,
    ...
    if (ramp_up_dist + ramp_down_dist > path_length) {
        ...
        max_speed = sqrt((2 * max_acc * path_length + powf(start_speed, 2) +
                          powf(final_speed, 2)) /
                         2.0);
    ...
    if (abs(distance - (ramp_up_dist + plateau_dist + ramp_down_dist)) <
        0.00001) {
...
bool TrapezoidalMotion(double path_length, double max_speed, double max_acc,
                       double time_into_lap, double start_speed,
                       double final_speed, double& pos_out, double& speed_out) {
    ...
    if (time_into_lap < ramp_up_time) {
        ...
        pos_out = 0.5 * max_acc * time_into_lap * time_into_lap +
                  start_speed * time_into_lap;
        ...
        double time_into_ramp_down =
            time_into_lap - (ramp_up_time + plateau_time);
        pos_out = 0.5 * (-max_acc) * time_into_ramp_down * time_into_ramp_down +
                  max_speed * time_into_ramp_down +
                  (ramp_up_dist + plateau_dist);
