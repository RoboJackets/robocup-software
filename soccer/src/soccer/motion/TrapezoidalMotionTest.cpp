...
bool trapezoid1(double t, double& pos_out, double& speed_out) {
    ...
    bool valid =
        TrapezoidalMotion(path_length, max_speed, max_acc, t, start_speed,
                          final_speed, pos_out, speed_out);

    if (valid) {
        auto time = Trapezoidal::getTime(pos_out, path_length, max_speed,
                                         max_acc, start_speed, final_speed);
...
bool trapezoid2(double t, double& pos_out, double& speed_out) {
    ...
    if (valid) {
        double time = Trapezoidal::getTime(pos_out, path_length, max_speed,
                                           max_acc, start_speed, final_speed);
        EXPECT_NEAR(time, t, 0.001);
    }
    return valid;
