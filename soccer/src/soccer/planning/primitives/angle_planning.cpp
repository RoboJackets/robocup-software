...
void plan_angles(Trajectory* trajectory, const RobotInstant& start_instant,
    ...
    if (trajectory->empty()) {
        ...
        double time = Trapezoidal::get_time(delta_angle, delta_angle, constraints.max_speed,
                                            constraints.max_accel, 0, 0);

        SPDLOG_INFO("delta angle: {}", delta_angle);
        for (int i = 1; i < time / TIME_STEP; i++) {
            double pos_out = 0;
            double speed_out = 0;
            Trapezoidal::trapezoidal_motion(delta_angle, constraints.max_speed,
