    ...
    *trajectory = trajectory->sub_trajectory(start_time, trajectory->end_time());
    ...
    for (int i = 1; i < trajectory->num_instants(); i++) {
        ...
        if (std::isnan(velocity.at(i))) {
            SPDLOG_INFO("SFSFSFS");
            SPDLOG_INFO("1: {}, {}", trajectory->instant_at(i).linear_velocity().x(),
                        trajectory->instant_at(i).linear_velocity().y());
            SPDLOG_INFO("2: {}, {}", rj_geometry::Point(gradient).x(),
                        rj_geometry::Point(gradient).y());
        }
    }
