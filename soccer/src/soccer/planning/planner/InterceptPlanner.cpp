...
Trajectory InterceptPlanner::plan(const PlanRequest& plan_request) {
    InterceptCommand command =
        std::get<InterceptCommand>(plan_request.motionCommand);
    ...
    RJ::Seconds ball_to_point_time =
        ball.query_seconds_near(command.target, &target_pos_on_line);

    // vector from robot to target
    Geometry2d::Point bot_to_target =
        (target_pos_on_line - start_instant.position());
    ...
    double max_speed = std::min(
        start_instant.linear_velocity().mag() +
            sqrt(2 * motion_constraints.maxAcceleration * bot_to_target.mag()),
    ...
    for (int i = 0; i <= num_iterations; i++) {
        ...
        LinearMotionInstant final_stopping_motion{
            target_pos_on_line, mag * max_speed * bot_to_target.normalized()};

        trajectory = CreatePath::simple(
            start_instant.linear_motion(), final_stopping_motion,
        ...
        if (trajectory.duration() <= ball_to_point_time) {
            ...
            PlanAngles(&trajectory, start_instant,
    ...
    PlanAngles(&trajectory, start_instant, AngleFns::facePoint(ball.position),
               plan_request.constraints.rot);
    trajectory.stamp(RJ::now());
