...
DEFINE_NS_FLOAT64(kPlanningParamModule, replanner, pos_change_threshold, 0.025,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, replanner, vel_change_threshold, 0.025,
                  "Velocity target threshold (m/s); if the change in velocity from the originally planned "
                  "target is smaller than this amount no replanning will occur.");
DEFINE_NS_FLOAT64(kPlanningParamModule, replanner, partial_replan_lead_time, 0.1,
                  "Portion of the path (seconds) that will not be changed in the case of a partial replan");
DEFINE_NS_FLOAT64(kPlanningParamModule, replanner, off_path_threshold, 0.5,
                  "Position error threshold (m), a partial replan will be forced if we are not within this "
                  "amount of the planned trajectory.");

DEFINE_NS_BOOL(kPlanningParamModule, rrt, enable_debug_drawing, false, "Whether to enable RRT debug drawing");
...
DEFINE_NS_FLOAT64(kPlanningParamModule, collect, approach_accel_scale, 0.7,
...
// This should be almost zero. Increase if the noise on vision causes problems and we never transition.
DEFINE_NS_FLOAT64(kPlanningParamModule, collect, dist_cutoff_to_control, 0.0285,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, collect, dist_cutoff_to_approach, 1,
...
// If we are trying to transition with a large delta velocity between the robot and the ball decrease this number.
DEFINE_NS_FLOAT64(kPlanningParamModule, collect, vel_cutoff_to_control, 0.3,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, collect, stop_dist_scale, 1,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, collect, target_point_lowpass_gain, 0.8,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, settle, ball_speed_percent_for_dampen, 0.1,
                  "How much of the ball seed to contact the ball with before slowing down to dampen the initial hit (unitless)")
...
DEFINE_NS_FLOAT64(kPlanningParamModule, settle, intercept_buffer_time, 0.3,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, settle, shortcut_dist, 0.09,
...
DEFINE_NS_FLOAT64(kPlanningParamModule, settle, max_ball_angle_for_reset, 20,
