...
PlanRequest PlannerForRobot::make_request(const RobotIntent& intent) {
     ...
     * at a fixed period, which we think correlates with (full) replanning. instead of starting new plans
     * with simply what vision sees as the current speed, we want to take into account the previously sent
     * setpoint. this is because during the time it takes for us to replan and radio, the robot is trying to get 
     * closer to the setpoint, so the sudden drop, which is intended to actually create a smooth curve
     * of velocities,  actually causes the robot to slow down suddenly. this is all an informed guess though. 
     * this behavior doesn't happen in sim. if this theory is even right, the sim difference is explained simply:
     * sim robots can accelerate perfectly, whereas real firmware and hardware are slower. also radio/vision latency
     * causes there to be a larger difference in what we saw in vision when we started planning and what speed
     * the robot made it to when it gets the new plan.
     * 
     */
