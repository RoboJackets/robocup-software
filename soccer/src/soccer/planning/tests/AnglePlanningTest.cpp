...
static Trajectory make_straight_line_trajectory() {
    ...
    trajectory.append_instant(RobotInstant{Pose(0, 0, 2), Twist(1, 0, 0), RJ::Time(0s)});
    trajectory.append_instant(RobotInstant{Pose(0.5, 0, 0), Twist(1, 0, 0), RJ::Time(500ms)});
    trajectory.append_instant(RobotInstant{Pose(1, 0, 0), Twist(1, 0, 0), RJ::Time(1000ms)});
    trajectory.append_instant(RobotInstant{Pose(1.5, 0, 0), Twist(1, 0, 0), RJ::Time(1500ms)});
    trajectory.append_instant(RobotInstant{Pose(2, 0, 0), Twist(1, 0, 0), RJ::Time(2000ms)});
    trajectory.append_instant(RobotInstant{Pose(2.5, 0, 0), Twist(1, 0, 0), RJ::Time(2500ms)});
    trajectory.append_instant(RobotInstant{Pose(3, 0, 0), Twist(1, 0, 0), RJ::Time(3000ms)});
    return trajectory;
}

static RotationConstraints make_rotation_constraints() {
    RotationConstraints constraints;
    constraints.max_accel = 1.0;
...
TEST(AnglePlanning, obeys_angle_function_target) {
    ...
    auto angle_fn = AngleFns::face_point(Point(2, 0.5));
    ...
    plan_angles(&trajectory, initial_instant, angle_fn, constraints);
