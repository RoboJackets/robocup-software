...
public:
    void RunTest() {
        double dt = 0.005;
        double total_time = Trapezoid::timeRemaining(initial, goal, max_velocity, max_acceleration);

        Trapezoid::State previous =
            Trapezoid::predictIn(initial, goal, max_velocity, max_acceleration, 0);

        // NOLINTNEXTLINE
        for (double t = dt; t <= total_time; t += dt) {
            Trapezoid::State next =
                Trapezoid::predictIn(initial, goal, max_velocity, max_acceleration, t);
        ...
        Trapezoid::State final =
            Trapezoid::predictIn(initial, goal, max_velocity, max_acceleration, total_time);
