...
#include "seeker_request.hpp"
#include "join_marking_request.hpp"
#include "leave_marking_request.hpp"
...
*/
using AgentRequest = std::variant<IncomingBallRequest, PassRequest, KickerRequest, ScorerRequest, JoinWallRequest, LeaveWallRequest, ResetScorerRequest, BallInTransitRequest, TestRequest, PositionRequest, SeekerRequest, JoinMarkingRequest, LeaveMarkingRequest>;
...
struct RosConverter<strategy::communication::AgentRequest, rj_msgs::msg::AgentRequest> {
	static rj_msgs::msg::AgentRequest to_ros(const strategy::communication::AgentRequest& from) {
		...
		if (const auto* incoming_ball_request = std::get_if<strategy::communication::IncomingBallRequest>(&from)) {
		...
		} else if (const auto* seeker_request = std::get_if<strategy::communication::SeekerRequest>(&from)) {
			result.seeker_request.emplace_back(convert_to_ros(*seeker_request));
		} else if (const auto* join_marking_request = std::get_if<strategy::communication::JoinMarkingRequest>(&from)) {
			result.join_marking_request.emplace_back(convert_to_ros(*join_marking_request));
		} else if (const auto* leave_marking_request = std::get_if<strategy::communication::LeaveMarkingRequest>(&from)) {
			result.leave_marking_request.emplace_back(convert_to_ros(*leave_marking_request));
		} else {
	...
	static strategy::communication::AgentRequest from_ros(const rj_msgs::msg::AgentRequest& from) {
		...
		if (!from.incoming_ball_request.empty()) {
		...
		} else if (!from.join_marking_request.empty()) {
			result = convert_from_ros(from.join_marking_request.front());
		} else if (!from.leave_marking_request.empty()) {
			result = convert_from_ros(from.leave_marking_request.front());
		} else {
			throw std::runtime_error("Invalid variant of AgentRequest");
		}
		return result;
...
struct RosConverter<strategy::communication::AgentResponse, rj_msgs::msg::AgentResponse> {
	static rj_msgs::msg::AgentResponse to_ros(const strategy::communication::AgentResponse& from) {
		...
		if (const auto* scorer_response = std::get_if<strategy::communication::ScorerResponse>(&(from.response))) {
