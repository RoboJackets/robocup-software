...
*/
using AgentRequest = std::variant<JoinWallRequest, TestRequest, PassRequest, ScorerRequest, BallInTransitRequest, SeekerRequest, PositionRequest, LeaveWallRequest, ResetScorerRequest, IncomingBallRequest>;

/**
* @brief a conglomeration of the different response types.
*/
using AgentResponseVariant = std::variant<ScorerResponse, LeaveWallResponse, PositionResponse, TestResponse, PassResponse, Acknowledge, JoinWallResponse>;
...
struct RosConverter<strategy::communication::AgentRequest, rj_msgs::msg::AgentRequest> {
	static rj_msgs::msg::AgentRequest to_ros(const strategy::communication::AgentRequest& from) {
		...
		if (const auto* join_wall_request = std::get_if<strategy::communication::JoinWallRequest>(&from)) {
