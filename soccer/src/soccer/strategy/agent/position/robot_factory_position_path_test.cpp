#include "robot_factory_position_path_test.hpp"

namespace strategy {

RobotFactoryPositionPathTest::RobotFactoryPositionPathTest(int r_id)
    : Position(r_id, "RobotFactoryPositionPathTest") {
    if (robot_id_ == 0) {
...
std::optional<RobotIntent> RobotFactoryPositionPathTest::get_task(
    WorldState& world_state, FieldDimensions& field_dimensions) {
...
std::optional<RobotIntent> RobotFactoryPositionPathTest::derived_get_task(
...
RobotFactoryPositionPathTest::send_communication_request() {
    // Call to super
    return current_position_->send_communication_request();
}

void RobotFactoryPositionPathTest::receive_communication_response(
    communication::AgentPosResponseWrapper response) {
    // Call to super
    current_position_->receive_communication_response(response);
}

communication::PosAgentResponseWrapper RobotFactoryPositionPathTest::receive_communication_request(
    communication::AgentPosRequestWrapper request) {
    // Return the response
    return current_position_->receive_communication_request(request);
}

void RobotFactoryPositionPathTest::derived_acknowledge_pass() {
    current_position_->derived_acknowledge_pass();
}

void RobotFactoryPositionPathTest::derived_pass_ball() { current_position_->derived_pass_ball(); }

void RobotFactoryPositionPathTest::derived_acknowledge_ball_in_transit() {
    current_position_->derived_acknowledge_ball_in_transit();
}

void RobotFactoryPositionPathTest::set_is_done() { current_position_->set_is_done(); }

void RobotFactoryPositionPathTest::die() { current_position_->die(); }

void RobotFactoryPositionPathTest::revive() { current_position_->revive(); }

std::string RobotFactoryPositionPathTest::get_current_state() {
