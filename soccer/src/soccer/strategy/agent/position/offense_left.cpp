#include "offense_left.hpp"

namespace strategy {

OffenseLeft::OffenseLeft(int r_id) : Position{r_id, "OffenseLeft"}, seeker_{r_id} {}

OffenseLeft::OffenseLeft(const Position& other) : Position{other}, seeker_{robot_id_} {
    position_name_ = "OffenseLeft";
}

std::optional<RobotIntent> OffenseLeft::derived_get_task(RobotIntent intent) {
...
std::string OffenseLeft::get_current_state() {
    return std::string{"OffenseLeft"} + std::to_string(static_cast<int>(current_state_));
}

OffenseLeft::State OffenseLeft::next_state() {
...
std::optional<RobotIntent> OffenseLeft::state_to_task(RobotIntent intent) {
    switch (current_state_) {
        case DEFAULT: {
...
bool OffenseLeft::check_if_open(int target_robot_shell) {
...
communication::PosAgentResponseWrapper OffenseLeft::receive_communication_request(
...
void OffenseLeft::receive_communication_response(communication::AgentPosResponseWrapper response) {
...
void OffenseLeft::derived_acknowledge_pass() {
    // I have been chosen as the receiver
    current_state_ = RECEIVING_START;
    reset_timeout();
}

void OffenseLeft::derived_pass_ball() {
...
void OffenseLeft::derived_acknowledge_ball_in_transit() {
    // The ball is coming to me
    current_state_ = RECEIVING;
    reset_timeout();
}

bool OffenseLeft::has_open_shot() const {
    // Ball position
...
double OffenseLeft::distance_from_their_robots(rj_geometry::Point tail, rj_geometry::Point head) const {
...
bool OffenseLeft::can_steal_ball() const {
...
rj_geometry::Point OffenseLeft::calculate_best_shot() const {
    // Goal location
...
bool OffenseLeft::ball_in_red() const {
...
void OffenseLeft::broadcast_seeker_request(rj_geometry::Point seeking_point, bool adding) {
