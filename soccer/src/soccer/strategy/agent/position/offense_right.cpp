#include "offense_right.hpp"

namespace strategy {

OffenseRight::OffenseRight(int r_id) : Position{r_id, "OffenseRight"}, seeker_{r_id} {}

OffenseRight::OffenseRight(const Position& other) : Position{other}, seeker_{robot_id_} {
    position_name_ = "OffenseRight";
}

std::optional<RobotIntent> OffenseRight::derived_get_task(RobotIntent intent) {
...
std::string OffenseRight::get_current_state() {
    return std::string{"OffenseRight"} + std::to_string(static_cast<int>(current_state_));
}

OffenseRight::State OffenseRight::next_state() {
...
std::optional<RobotIntent> OffenseRight::state_to_task(RobotIntent intent) {
    switch (current_state_) {
        case DEFAULT: {
...
bool OffenseRight::check_if_open(int target_robot_shell) {
...
communication::PosAgentResponseWrapper OffenseRight::receive_communication_request(
...
void OffenseRight::receive_communication_response(communication::AgentPosResponseWrapper response) {
...
void OffenseRight::derived_acknowledge_pass() {
    // I have been chosen as the receiver
    current_state_ = RECEIVING_START;
    reset_timeout();
}

void OffenseRight::derived_pass_ball() {
...
void OffenseRight::derived_acknowledge_ball_in_transit() {
    // The ball is coming to me
    current_state_ = RECEIVING;
    reset_timeout();
}

bool OffenseRight::has_open_shot() const {
    // Ball position
...
double OffenseRight::distance_from_their_robots(rj_geometry::Point tail, rj_geometry::Point head) const {
...
bool OffenseRight::can_steal_ball() const {
...
rj_geometry::Point OffenseRight::calculate_best_shot() const {
    // Goal location
...
bool OffenseRight::ball_in_red() const {
...
void OffenseRight::broadcast_seeker_request(rj_geometry::Point seeking_point, bool adding) {
