#include "goalie_offense.hpp"

namespace strategy {

GoalieOffense::GoalieOffense(int r_id) : Position{r_id, "GoalieOffense"}, seeker_{r_id} {}

GoalieOffense::GoalieOffense(const Position& other) : Position{other}, seeker_{robot_id_} {
    position_name_ = "GoalieOffense";
}

std::optional<RobotIntent> GoalieOffense::derived_get_task(RobotIntent intent) {
...
std::string GoalieOffense::get_current_state() {
    return std::string{"GoalieOffense"} + std::to_string(static_cast<int>(current_state_));
}

GoalieOffense::State GoalieOffense::next_state() {
...
std::optional<RobotIntent> GoalieOffense::state_to_task(RobotIntent intent) {
    switch (current_state_) {
        case DEFAULT: {
        ...
        case SEEKING: {
            planning::LinearMotionInstant target{seeker_point};
...
bool GoalieOffense::check_if_open(int target_robot_shell) {
...
communication::PosAgentResponseWrapper GoalieOffense::receive_communication_request(
...
void GoalieOffense::receive_communication_response(communication::AgentPosResponseWrapper response) {
...
void GoalieOffense::derived_acknowledge_pass() {
    // I have been chosen as the receiver
    current_state_ = RECEIVING_START;
    reset_timeout();
}

void GoalieOffense::derived_pass_ball() {
...
void GoalieOffense::derived_acknowledge_ball_in_transit() {
    // The ball is coming to me
    current_state_ = RECEIVING;
    reset_timeout();
}

bool GoalieOffense::has_open_shot() const {
    // Ball position
...
double GoalieOffense::distance_from_their_robots(rj_geometry::Point tail, rj_geometry::Point head) const {
...
bool GoalieOffense::can_steal_ball() const {
    // Ball in red zone or not
    if (ball_in_red()) {
        return false;
    }

    if (current_play_state_.is_restart()) {
        return false;
    }
    ...
    for (auto enemy : this->last_world_state_->their_robots) {
        ...
        if (dist < our_dist && dist < kStealBallRadius) {
            closest = false;
            break;
        }
    }
...
rj_geometry::Point GoalieOffense::calculate_best_shot() const {
    // Goal location
...
bool GoalieOffense::ball_in_red() const {
...
void GoalieOffense::broadcast_seeker_request(rj_geometry::Point seeking_point, bool adding) {
