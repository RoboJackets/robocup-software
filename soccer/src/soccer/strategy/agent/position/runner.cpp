...
planning::LinearMotionInstant vertices[4] = {
    planning::LinearMotionInstant({-2, 8}, {0, 0}), planning::LinearMotionInstant({-2, 3}, {0, 0}),
    planning::LinearMotionInstant({0, 3}, {0, 0}), planning::LinearMotionInstant({0, 8}, {0, 0})};
...
std::optional<RobotIntent> strategy::Runner::state_to_task(RobotIntent intent) {
    if (current_state_ == RUNNING) {
        ...
        intent.motion_command = planning::MotionCommand{"path_target", vertices[current_vertex],
