...
KickResults KickEvaluator::eval_pt_to_seg(Point origin, Segment target) {
    ...
    function<tuple<float, float>(float)> keFunc = bind(
        &eval_calculation, std::placeholders::_1, (kick_mean->value()), (kick_std_dev->value()),
        cref(botMeans), cref(botStDevs), cref(botVertScales), targetWidth / -2, targetWidth / 2);
    ...
    KickEvaluator::init_gradient_configs(parallelConfig, keFunc, botMeans, botStDevs,
...
tuple<float, float> KickEvaluator::eval_calculation(float x, float kmean, float kstdev,
                                                    const vector<float>& robotMeans,
                                                    const vector<float>& robotStDevs,
                                                    const vector<float>& robotVertScales,
    ...
    fterm = fast_exp(-0.5f * (kx + rmean) * (kx + rmean) / (kstdev2 + rstdev2)) * robotV * sqrt2pi *
            (kx + rmean);
...
vector<tuple<float, float> > KickEvaluator::convert_robots_to_polar(Point origin, Point target) {
    ...
    transform(
        bots.begin(), bots.end(), back_inserter(botLocations),
    ...
    transform(hypothetical_robot_locations.begin(), hypothetical_robot_locations.end(),
              back_inserter(botLocations), [target, origin, this](Point obstacle) {
...
void KickEvaluator::init_gradient_configs(ParallelGradient1DConfig& pConfig,
    ...
    for (tuple<float, float> xStart : xStarts) {
        pConfig.GA1DConfig.emplace_back(&func, get<1>(xStart), get<0>(xStart), dxError,
                                        maxXMovement, temperatureDescent, temperatureMin,
                                        maxIterations, maxValue, maxThresh);
    }

    pConfig.xCombineThresh = static_cast<float>(
        *min_element(robotStDevs.begin(), robotStDevs.end()) * *start_x_offset / 2);
}
