...
void OurRobot::createConfiguration(Configuration *cfg) {
  ...
  _oppAvoidRadius =
      new ConfigDouble(cfg, "PathPlanner/oppAvoidRadius", Robot_Radius - 0.01);
...
std::shared_ptr<Geometry2d::Shape> OurRobot::createBallObstacle() const {
  ...
  if (_state->gameState.state != GameState::Playing &&
    ...
    return std::shared_ptr<Geometry2d::Shape>(new Circle(
        _state->ball.pos, Field_Dimensions::Current_Dimensions.CenterRadius()));
  }
...
Geometry2d::ShapeSet
OurRobot::collectAllObstacles(const Geometry2d::ShapeSet &globalObstacles) {
