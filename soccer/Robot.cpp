...
void OurRobot::_chip(uint8_t strength) {
	...
	Segment mid_field_line = Segment(Point(-Field_Dimensions.Width /2,Field_Dimensions.Length /2), Point(Field_Dimensions.Width /2,Field_Dimensions.Length /2));
	Point intersection;
...
std::shared_ptr<Geometry2d::Shape> OurRobot::createBallObstacle() const {
	...
	{
		return std::shared_ptr<Geometry2d::Shape>(new Circle(_state->ball.pos, Field_Dimensions.CenterRadius));
	}
