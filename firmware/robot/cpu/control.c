...
static const int w0x8 = 136733;  // these integers come from the trig functions
                                 // of the angles of the robots' wheels,
...
static void dumb_update() {
    int wheel_command[4];
    if (base2008) {  // this is because integer math is faster than simply
                     // handling doubles
        wheel_command[0] =
            (-cmd_body_x * w0x8 / 100000 - cmd_body_y * w0y8 / 100000) *
                100000 / (w0x8 + w0y8) +
            cmd_body_w;
        wheel_command[1] =
            (-cmd_body_x * w1x8 / 100000 + cmd_body_y * w1y8 / 100000) *
                100000 / (w1x8 + w1y8) +
            cmd_body_w;
        wheel_command[2] =
            (cmd_body_x * w2x8 / 100000 + cmd_body_y * w2y8 / 100000) * 100000 /
                (w2x8 + w2y8) +
            cmd_body_w;
        wheel_command[3] =
            (cmd_body_x * w3x8 / 100000 - cmd_body_y * w3y8 / 100000) * 100000 /
                (w3x8 + w3y8) +
            cmd_body_w;
    } else {
        wheel_command[0] =
            (-cmd_body_x * wrx11 / 100000 - cmd_body_y * wry11 / 100000) *
                100000 / (wrx11 + wry11) +
            cmd_body_w;
        wheel_command[1] =
            (-cmd_body_x * wfx11 / 100000 + cmd_body_y * wfy11 / 100000) *
                100000 / (wfx11 + wfy11) +
            cmd_body_w;
        wheel_command[2] =
            (cmd_body_x * wfx11 / 100000 + cmd_body_y * wfy11 / 100000) *
...
static void step_debug() {
    ...
    printf("%2d.%03d %4d %3d %3d %2d", supply_mv / 1000, supply_mv % 1000,
           encoder_delta[step_motor], hall_delta[step_motor],
           motor_out[step_motor], dtime);
}
...
static void step_init(int argc, const char* argv[]) {
    ...
    if (argc < 2 || step_motor < 0 || step_motor > 4 ||
        step_level < -MOTOR_MAX || step_level > MOTOR_MAX) {
...
static void step_update() {
    ...
    if (failed && encoder_delta[step_motor] == 0 &&
        hall_delta[step_motor] == 0) {
        // Motor died
        printf("Motor failed\n");
        controller = 0;
    }
...
static void pid_update() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        //  delta = kp * delta_error + ki * error + kd * (delta_error -
        //  last_delta[i]);
        ...
        // Limit the change between consecutive cycles to prevent excessive
        // current
        ...
        if (i == pd_debug) {
            printf("%04x %04x -> %08x %08x %08x -> %08x\n", wheel_command[i],
                   speed, error, error1[i], error2[i], last_out[i]);
        }
