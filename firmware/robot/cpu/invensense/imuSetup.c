        ...
        for (i = 0; i < 4; i++) {
            if (mlCtrlParams.functions & ML_GRID) {
                ...
                mlCtrlxData.mlGridNumDMP[i] =
                    mlCtrlxData.mlGridNumDMP[i] / 2 + 1073741824;
                mlCtrlxData.controlInt[i] =
                    (mlCtrlxData.mlGridNumDMP[i] %
                     (128 * mlCtrlParams.gridThreshold[i])) /
                    128;
                gridTmp = mlCtrlxData.mlGridNumDMP[i] /
                          (128 * mlCtrlParams.gridThreshold[i]);
                tmp = 1 + 16777216L / mlCtrlParams.gridThreshold[i];
                mlCtrlxData.gridChange[i] =
                    gridTmp - mlCtrlxData.lastGridNum[i];
                if (mlCtrlxData.gridChange[i] > tmp / 2) {
                    mlCtrlxData.gridChange[i] =
                        gridTmp - tmp - mlCtrlxData.lastGridNum[i];
                } else if (mlCtrlxData.gridChange[i] < -tmp / 2) {
                    mlCtrlxData.gridChange[i] =
                        gridTmp + tmp - mlCtrlxData.lastGridNum[i];
                }
                if ((mlCtrlParams.functions & ML_HYSTERESIS) &&
                    (mlCtrlxData.gridChange[i] != 0)) {
                    if (mlCtrlxData.gridChange[i] > 0) {
                        mlCtrlxData.gridNumOffset[i] +=
                            128 * mlCtrlParams.gridThreshold[i];
                        mlCtrlxData.controlInt[i] =
                            mlCtrlParams.gridThreshold[i] / 2;
                    }
                    if (mlCtrlxData.gridChange[i] < 0) {
                        mlCtrlxData.gridNumOffset[i] -=
                            128 * mlCtrlParams.gridThreshold[i];
                ...
                if ((mlCtrlParams.gridCallback) &&
                    (mlCtrlxData.gridChange[i] != 0)) {
                    mlCtrlParams.gridCallback((ML_CONTROL_1 << i),
                                              mlCtrlxData.gridNum,
                                              mlCtrlxData.gridChange);
                }
...
tMLError MLGetDMPData(void) {
    ...
    if (MLDLGetFifoStatus() != 0) return ML_ERROR;
 ...
 *  @param[in] orientation A 9 element matrix that represents how the
 * accelerometers
...
tMLError MLSetAccelCalibration(float range, float* orientation) {
...

    {
        ...
        for (i = 0; i < 3; i++) {
            maxVal = 0;
            tmpSign = 0;
            if (mlxData.mlAccelCal[0 + 3 * i] < 0) tmpSign = 1;
            if (abs(mlxData.mlAccelCal[1 + 3 * i]) >
                abs(mlxData.mlAccelCal[0 + 3 * i])) {
                maxVal = 1;
                if (mlxData.mlAccelCal[1 + 3 * i] < 0) tmpSign = 1;
            }
            if (abs(mlxData.mlAccelCal[2 + 3 * i]) >
                abs(mlxData.mlAccelCal[1 + 3 * i])) {
                tmpSign = 0;
                maxVal = 2;
                if (mlxData.mlAccelCal[2 + 3 * i] < 0) tmpSign = 1;
            }
            if (maxVal == 0) {
                regs[tmpPtr++] = tmpA;
                if (tmpSign) regs[tmpPtr + 2] |= 0x01;
            } else if (maxVal == 1) {
                regs[tmpPtr++] = tmpB;
                if (tmpSign) regs[tmpPtr + 2] |= 0x01;
            } else {
    ...
    if (MLDLSetMemoryMPU(KEY_D_0_108, 2, regs) != ML_SUCCESS) return ML_ERROR;

    sf = mlxData.mlAccelSens / 1024;
    regs[0] = (unsigned char)(sf / 256);
    regs[1] = (unsigned char)(sf % 256);
    if (MLDLSetMemoryMPU(KEY_D_0_96, 2, regs) != ML_SUCCESS) return ML_ERROR;
...
tMLError MLSetGyroCalibration(float range, float* orientation) {
...

    {
        ...
        for (i = 0; i < 3; i++) {
            maxVal = 0;
            tmpSign = 0;
            if (mlxData.mlGyroOrient[0 + 3 * i] < 0) tmpSign = 1;
            if (abs(mlxData.mlGyroOrient[1 + 3 * i]) >
                abs(mlxData.mlGyroOrient[0 + 3 * i])) {
                maxVal = 1;
                if (mlxData.mlGyroOrient[1 + 3 * i] < 0) tmpSign = 1;
            }
            if (abs(mlxData.mlGyroOrient[2 + 3 * i]) >
                abs(mlxData.mlGyroOrient[1 + 3 * i])) {
                tmpSign = 0;
                maxVal = 2;
                if (mlxData.mlGyroOrient[2 + 3 * i] < 0) tmpSign = 1;
            }
            if (maxVal == 0) {
                regs[tmpPtr++] = tmpD;
                if (tmpSign) regs[tmpPtr + 2] |= 0x01;
        ...
        sf = (long)(((long long)mlxData.mlGyroSens * 767603923LL) /
                    1073741824LL);
...
tMLError MLSetInterrupts(uint_fast8_t interrupts) {
    ...
    if ((interrupts & (~intMask)) != 0) return ML_ERROR_INVALID_PARAMETER;
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_6, 1, regs) != ML_SUCCESS) return ML_ERROR;
...
tMLError MLSetDataMode(unsigned short dataMode) {
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_19, 1, regs) != ML_SUCCESS) return ML_ERROR;
...
unsigned char MLUpdateBias(void) {
    ...
    if (mlParams.biasUpdateFunc & ML_BIAS_FROM_NO_MOTION) {
        ...
        for (i = 0; i < 3; i++) {
            biasTmp2[i] =
                (((long)regs[i * 4] << 24) + ((long)regs[i * 4 + 1] << 16) +
                 ((long)regs[i * 4 + 2] << 8) + ((long)regs[i * 4 + 3]));
        }
        ...
        for (i = 0; i < 3; ++i) {
            biasTmp[i] =
                (long)(biasTmp2[0] * (float)mlxData.mlGyroOrient[i] /
                           (1L << 30) +
                       biasTmp2[1] * (float)mlxData.mlGyroOrient[i + 3] /
                           (1L << 30) +
                       biasTmp2[2] * (float)mlxData.mlGyroOrient[i + 6] /
                           (1L << 30));
        }
        regs[0] = DINAA0 + 15;
        if (MLDLSetMemoryMPU(KEY_FCFG_6, 1, regs) != ML_SUCCESS)
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (biasTmp[i] < 0) biasTmp[i] += 65536;
        ...
        biasTmp[1] = 0;
        biasTmp[2] = 0;
        if (MLDLSetMemoryMPU(KEY_D_0_36, 12, (unsigned char*)biasTmp) !=
            ML_SUCCESS)
            return ML_ERROR;
        if (MLDLSetMemoryMPU(KEY_D_0_52, 12, (unsigned char*)biasTmp) !=
            ML_SUCCESS)
            return ML_ERROR;
