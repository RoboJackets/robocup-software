...
tMLCTRLParams mlCtrlParams = {
    {MLCTRL_SENSITIVITY_0_DEFAULT, MLCTRL_SENSITIVITY_1_DEFAULT,
    ...
    {MLCTRL_PARAMETER_ARRAY_0_DEFAULT, MLCTRL_PARAMETER_ARRAY_1_DEFAULT,
     MLCTRL_PARAMETER_ARRAY_2_DEFAULT,
     MLCTRL_PARAMETER_ARRAY_3_DEFAULT},  // parameterArray
    {MLCTRL_PARAMETER_AXIS_0_DEFAULT, MLCTRL_PARAMETER_AXIS_1_DEFAULT,
     MLCTRL_PARAMETER_AXIS_2_DEFAULT,
     MLCTRL_PARAMETER_AXIS_3_DEFAULT},  // parameterAxis
    {MLCTRL_GRID_THRESHOLD_0_DEFAULT, MLCTRL_GRID_THRESHOLD_1_DEFAULT,
     MLCTRL_GRID_THRESHOLD_2_DEFAULT,
     MLCTRL_GRID_THRESHOLD_3_DEFAULT},  // gridThreshold
    {MLCTRL_GRID_MAXIMUM_0_DEFAULT, MLCTRL_GRID_MAXIMUM_1_DEFAULT,
 ...
 *          - ML_SMOOTH
 *          Indicates that noise from unintentional motion should be filtered
 * out.
 *          - ML_DEAD_ZONE
...
tMLError MLSetControlFunc(unsigned short function) {
    INVENSENSE_FUNC_START
    unsigned char regs[8] = {DINA06, DINA26, DINA46, DINA66,
                             DINA0E, DINA2E, DINA4E, DINA6E};
    unsigned char i;
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_4, 8, regs) != ML_SUCCESS) return ML_ERROR;
 ...
 *          high precision.
 *          MLGetControlSignal is used to acquire the current data of a control
 * signal.
 *          If ML_GRID is being used, MLGetGridNumber will probably be
 * preferrable.
 *
 ...
 *          - ML_CONTROL_4.
 *
 *  @param  reset   Indicates whether the control signal should be reset to
 * zero.
...
tMLError MLGetControlSignal(unsigned short controlSignal, unsigned short reset,
    ...
    if (isCompatible(IMU_C_OPEN | IMU_C_START,
                     IMU_C_OPEN_CALLED | IMU_C_START_CALLED) != ML_SUCCESS)
        return ML_ERROR;
 ...
 *          MLSetGridThresh is used to adjust the size of the grid being
 * controlled.
 *  @param  controlSignal   Indicates which control signal is being modified.
 *                          Must be one of:
 *                          - ML_CONTROL_1,
 ...
 *                          - ML_CONTROL_3 and
 *                          - ML_CONTROL_4.
 *  @param  threshold       The threshold of the control signal at which the
 * grid
...
/**
 *  @brief  MLSetGridMax is used to set the maximum grid number for a control
 * signal.
 *          MLSetGridMax is used to adjust the maximum allowed grid number,
 * above
 ...
 *          MLSetGridCallback.
 *
 *  @param  controlSignal   Indicates which control signal crossed a grid
 * threshold.
 *                          Must be one of:
 *                          - ML_CONTROL_1,
 *                          - ML_CONTROL_2,
 ...
 *                          - ML_CONTROL_4.
 *
 *  @param  gridNumber  An array of four numbers representing the grid number
 * for each
 *                      control signal.
 *  @param  gridChange  An array of four numbers representing the change in grid
 * number
 *                      for each control signal.
 *  @return Zero if the command is successful; an ML error code otherwise.
 */
typedef void (*GridCallbackPtr)(unsigned short controlSignal, int* gridNum,
                                int* gridChange);

/**
 *  @brief  MLSetGridCallback is used to register a callback function that
 *          will trigger when the grid location changes.
 *          MLSetGridCallback allows a user to define a callback function that
 will
...
/**
 *  @brief  MLSetControlSignals is used to register which control signals will
 * be
 *          processed. MLRegisterControlSignals is used to register which
 * control
 ...
 *          MLDmpPedometerStandAloneOpen().
 *
 *  @param  controlSignals  A control signal or bitwise OR of multiple control
 * signals.
 *
 *  @return Zero if the command is successful; an ML error code otherwise.
 */
...
/**
 *  @brief  MLSetControlData is used to assign physical parameters to control
 * signals.
 *          MLSetControlData allows flexibility in assigning physical parameters
 * to
 *          control signals. For example, the user is allowed to use raw
 * gyroscope data
 *          as an input to the control algorithm.
 *          Alternatively, angular velocity can be used, which combines
 * gyroscopes and
 *          accelerometers to provide a more robust physical parameter. Finally,
 * angular
 *          velocity in world coordinates can be used, providing a control
 * signal in
 ...
 *                          - ML_CONTROL_4.
 *
 *  @param  parameterArray   Indicates which parameter array is being assigned
 * to a
 *                          control signal. Must be one of:
 *                          - ML_GYROS,
 *                          - ML_ANGULAR_VELOCITY, or
 *
 *  @param  parameterAxis   Indicates which axis of the parameter array will be
 * used.
 *                          Must be:
 *                          - ML_ROLL,
 *                          - ML_PITCH, or
