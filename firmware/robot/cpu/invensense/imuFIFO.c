...
int_fast8_t readAndProcessFIFO(int_fast8_t numPackets) {
    ...
    for (processed = 0; processed < numPackets; ++processed) {
        if (MLDLGetDMPData(DATASRC_FIFO, buf,
                           (unsigned char)FIFOData.mlFIFOPacketSize) !=
...
/**
 *  @brief  MLSetProcessedFIFOCallback is used to set a callback when a FIFO
 * data has
 *          been processed. This is useful because system slow down may cause
 * more than
 *          one FIFO buffer to be processed for a single interrupt. This would
 * keep things
 *          from dropping data.
 *
...
tMLError MLProcessFIFOData(const unsigned char* dmpData) {
    ...
    if (FIFOData.mlFIFODataConfig[ML_GYROS] & ML_32_BIT) {
        for (i = 0; i < 3; i++) {
            if ((FIFOData.mlFIFODataConfig[ML_GYROS] >> i) & 0x0001) {
                FIFOData.mlCalibratedData[i] =
                    (((long)dmpData[FIFOPtr] << 24) +
                     ((long)dmpData[FIFOPtr + 1] << 16) +
                     ((long)dmpData[FIFOPtr + 2] << 8) +
                     ((int)dmpData[FIFOPtr + 3]));
                FIFOData.mlCalibratedData[i] =
                    (long)(((long long)FIFOData.mlCalibratedData[i] * 65536L) /
    ...
    if (FIFOData.mlFIFODataConfig[ML_ACCELS] & ML_32_BIT) {
        for (i = 0; i < 3; i++) {
            if ((FIFOData.mlFIFODataConfig[ML_ACCELS] >> i) & 0x0001) {
                ...
                if (mlxData.mlAccelSens != 0) {
                    FIFOData.mlCalibratedData[i + 3] /=
                        (0x20000000 / mlxData.mlAccelSens);
                }
                FIFOPtr += 4;
            }
        }
        ...
        for (i = 0; i < 3; i++) {
            if ((FIFOData.mlFIFODataConfig[ML_ACCELS] >> i) & 0x0001) {
                ...
                if (mlxData.mlAccelSens != 0) {
                    FIFOData.mlCalibratedData[i + 3] /=
                        (0x20000000UL / mlxData.mlAccelSens);
                }
                FIFOPtr += 2;
            }
        }
    ...
    if (FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION] & ML_32_BIT) {
        for (i = 0; i < 3; i++) {
            if ((FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION] >> i) &
                0x0001) {
                FIFOData.mlLinearAccBody[i] =
                    (((long)dmpData[FIFOPtr] << 24) +
                     ((long)dmpData[FIFOPtr + 1] << 16) +
                     ((long)dmpData[FIFOPtr + 2] << 8) +
                     ((int)dmpData[FIFOPtr + 3]));
                ...
                if (mlxData.mlAccelSens != 0) {
                    FIFOData.mlLinearAccBody[i] /=
                        (536870912L / mlxData.mlAccelSens);
                }
                FIFOPtr += 4;
            }
        }
    ...
    if (FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION_WORLD] & ML_32_BIT) {
        for (i = 0; i < 3; i++) {
            if ((FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION_WORLD] >> i) &
                0x0001) {
                FIFOData.mlLinearAccWorld[i] =
                    (((long)dmpData[FIFOPtr] << 24) +
                     ((long)dmpData[FIFOPtr + 1] << 16) +
                     ((long)dmpData[FIFOPtr + 2] << 8) +
                     ((int)dmpData[FIFOPtr + 3]));
                ...
                if (i == 2) FIFOData.mlLinearAccWorld[2] -= 65536L;
                FIFOPtr += 4;
            }
        }
    } else if (FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION_WORLD] &
               ML_16_BIT) {
        for (i = 0; i < 3; i++) {
            if ((FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION_WORLD] >> i) &
                0x0001) {
    ...
    if ((FIFOData.mlFIFODataConfig[ML_TEMPERATURE]) & 0x0001) {
        FIFOData.mlTemperature =
            (((int)dmpData[FIFOPtr] << 8) + ((int)dmpData[FIFOPtr + 1]));
        if (FIFOData.mlTemperature > 32768L) FIFOData.mlTemperature -= 65536L;
    ...
    for (i = 0; i < 6; i++) {
        if ((FIFOData.mlFIFODataConfig[ML_RAW_DATA] >> i) & 0x0001) {
            ...
            FIFOData.mlSensorData[i] =
                (((int)dmpData[FIFOPtr + 1] << 8) + ((int)dmpData[FIFOPtr]));
    ...
    if (FIFOData.FIFOProcessCB) FIFOData.FIFOProcessCB();
...
tMLError IMUsendQuaternionToFIFO(uint_fast8_t accuracy) {
    ...
    if (accuracy == ML_16_BIT) regs[0] = DINAF8 + 2;

    if (MLDLSetMemoryMPU(KEY_CFG_8, 5, regs) != ML_SUCCESS) return ML_ERROR;
 ...
 * them
 *            or ML_ELEMENT_1, ML_ELEMENT_2, ML_ELEMENT_3 ... ML_ELEMENT_7 or'd
 * together
 *            for a subset. The first element is temperature, the next 3 are
 * gyro data,
 *            and the last 3 accel data.
 */
tMLError IMUsendRawToFIFO(uint_fast8_t elements) {
    INVENSENSE_FUNC_START
    unsigned char regs[13] = {DINAF8 + 3, DINAF8 + 3, DINAA0 + 3, DINAF8 + 2,
                              DINAA0 + 3, DINAF8 + 3, DINAA0 + 3, DINAF8 + 2,
                              DINAA0 + 3, DINAF8 + 3, DINAA0 + 3, DINAF8 + 2,
                              DINAA0 + 3};
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_15, 13, regs) != ML_SUCCESS) return ML_ERROR;
    if (elements & 0x7e)
        FIFOData.mlFIFODataConfig[ML_RAW_DATA] =
            (0x3f & (elements >> 1)) | ML_16_BIT;
    if (elements & 1) FIFOData.mlFIFODataConfig[ML_TEMPERATURE] = 1 | ML_16_BIT;
...
tMLError IMUgetAngularVelocityWorld(long* data) {
    ...
    data[0] =
        (long)(((long long)rot[0] * gyro[0] + (long long)rot[1] * gyro[1] +
                (long long)rot[2] * gyro[2]) >>
               30);
    data[1] =
        (long)(((long long)rot[3] * gyro[0] + (long long)rot[4] * gyro[1] +
                (long long)rot[5] * gyro[2]) >>
               30);
    data[2] =
        (long)(((long long)rot[6] * gyro[0] + (long long)rot[7] * gyro[1] +
                (long long)rot[8] * gyro[2]) >>
               30);
    return ML_SUCCESS;
}
...
tMLError IMUsendGyroToFIFO(uint_fast8_t elements, uint_fast8_t accuracy) {
    ...
    if (isCompatible(IMU_C_OPEN | IMU_C_START, IMU_C_OPEN_CALLED) != ML_SUCCESS)
        return ML_ERROR;
    if (accuracy == ML_16_BIT) regs[0] = DINAF8 + 2;
    if ((elements & ML_ELEMENT_1) == 0) regs[1] = DINAA0 + 3;
    if ((elements & ML_ELEMENT_2) == 0) regs[2] = DINAA0 + 3;
    if ((elements & ML_ELEMENT_3) == 0) regs[3] = DINAA0 + 3;

    if (MLDLSetMemoryMPU(KEY_CFG_9, 4, regs) != ML_SUCCESS) return ML_ERROR;
    FIFOData.mlFIFODataConfig[ML_GYROS] = (0x0007 & elements) | accuracy;
    return IMUsetFIFOFooter();
}

/** Sends accelerometer data to the FIFO. Accelerometer data is a 3 length
 * vector
 *  of 32-bits. Should be called once after IMUopen() and before IMUstart().
 *  @param[in] elements Which of the 3 elements to send. Use ML_ALL for all of
 * them
...
tMLError IMUsendAccelToFIFO(uint_fast8_t elements, uint_fast8_t accuracy) {
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_10, 4, regs) != ML_SUCCESS) return ML_ERROR;
 ...
 *  with gravity removed.  Should be called once after IMUopen() and before
 * IMUstart().
 *  @param[in] elements Which of the 3 elements to send. Use ML_ALL for all of
 * them
 *            or ML_ELEMENT_1, ML_ELEMENT_2, ML_ELEMENT_3 or'd together
 ...
 *             bit data.
 */
tMLError IMUsendLinearAccelToFIFO(uint_fast8_t elements,
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_12, 4, regs) != ML_SUCCESS) return ML_ERROR;
    FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION] =
        (0x0007 & elements) | accuracy;
    return IMUsetFIFOFooter();
}

/** Sends linear world accelerometer data to the FIFO. Linear world
 * accelerometer
 *  data is a 3 length vector of 32-bits. It is the acceleration in the world
 * frame
 *  with gravity removed. Should be called once after IMUopen() and before
 * IMUstart().
 *  @param[in] elements Which of the 3 elements to send. Use ML_ALL for all of
 ...
 *             bit data.
 */
tMLError IMUsendLinearAccelWorldToFIFO(uint_fast8_t elements,
                                       uint_fast8_t accuracy) {
    INVENSENSE_FUNC_START
    ...
    if (accuracy == ML_16_BIT) regs[0] = DINAF8 + 2;
    if ((elements & ML_ELEMENT_1) == 0) regs[1] = DINAA0 + 3;
    if ((elements & ML_ELEMENT_2) == 0) regs[3] = DINAA0 + 3;
    if ((elements & ML_ELEMENT_3) == 0) regs[3] = DINAA0 + 3;

    if (MLDLSetMemoryMPU(KEY_CFG_13, 6, regs) != ML_SUCCESS) return ML_ERROR;
    FIFOData.mlFIFODataConfig[ML_LINEAR_ACCELERATION_WORLD] =
        (0x0007 & elements) | accuracy;
    return IMUsetFIFOFooter();
}

/** Sends control data to the FIFO. Control data is a 4 length vector of
 * 32-bits.
 *  Should be called once after IMUopen() and before IMUstart().
 *  @param[in] elements Which of the 4 elements to send. Use ML_ALL for all of
 * them
 ...
 *             bit data.
 */
tMLError IMUsendControlDataToFIFO(uint_fast8_t elements,
                                  uint_fast8_t accuracy) {
    INVENSENSE_FUNC_START
    unsigned char regs[5] = {DINAF8 + 1, DINA20, DINA28, DINA30, DINA38};
    ...
    if ((elements & ML_ELEMENT_2) == 0) regs[2] = DINAA0 + 3;
    if ((elements & ML_ELEMENT_3) == 0) regs[3] = DINAA0 + 3;
    if ((elements & ML_ELEMENT_4) == 0) regs[3] = DINAA0 + 3;

    if (MLDLSetMemoryMPU(KEY_CFG_1, 5, regs) != ML_SUCCESS) return ML_ERROR;
...
static tMLError IMUsetFIFOFooter() {
    ...
    if (MLDLSetMemoryMPU(KEY_CFG_16, 1, &regs) != ML_SUCCESS) return ML_ERROR;
...
/**
 *  @brief      Returns 3-element vector of acclerometer data in body frame.
 *  @param[out] data    3-element vector of acclerometer data in body frame in
 * g's.
 ...
 *
 *          The DMP will add fifo entries every fifoRate + 1 IMU cycles (200
 * Hz).
 *          The following values apply:
 *
 *          <TABLE>
 *          <TR><TD>fifoRate</TD><TD>DMP Sample Rate</TD><TD>FIFO update
 * frequency</TD></TR>
...
void quaternionToRotationMatrix(const long* quat, long* rot) {
    rot[0] =
        (long)(((long long)quat[1] * quat[1] + (long long)quat[0] * quat[0]) /
                   536870912L -
               1073741824L);
    rot[1] =
        (long)(((long long)quat[1] * quat[2] - (long long)quat[3] * quat[0]) /
               536870912L);
    rot[2] =
        (long)(((long long)quat[1] * quat[3] + (long long)quat[2] * quat[0]) /
               536870912L);
    rot[3] =
        (long)(((long long)quat[1] * quat[2] + (long long)quat[3] * quat[0]) /
               536870912L);
    rot[4] =
        (long)(((long long)quat[2] * quat[2] + (long long)quat[0] * quat[0]) /
                   536870912L -
               1073741824L);
    rot[5] =
        (long)(((long long)quat[2] * quat[3] - (long long)quat[1] * quat[0]) /
               536870912L);
    rot[6] =
        (long)(((long long)quat[1] * quat[3] - (long long)quat[2] * quat[0]) /
               536870912L);
    rot[7] =
        (long)(((long long)quat[2] * quat[3] + (long long)quat[1] * quat[0]) /
               536870912L);
    rot[8] =
        (long)(((long long)quat[3] * quat[3] + (long long)quat[0] * quat[0]) /
                   536870912L -
               1073741824L);
}
