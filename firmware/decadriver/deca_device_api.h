...
#define DWT_SIG_TX_AA_DONE                                                     \
  6 // ACK frame has been sent (as a result of auto-ACK)
...
#define DWT_SIG_RX_SFDTIMEOUT                                                  \
  10 // Saw preamble but got no SFD within configured time
#define DWT_SIG_RX_PTOTIMEOUT                                                  \
  11 // Got preamble detection timeout (no preamble detected)
...
typedef struct {
  ...
  uint8
      nsSFD; //!< Boolean should we use non-standard SFD for better performance
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_readfromdevice // returns offset where requested data begins in supplied
                       // buffer, or, -1 for error.
    (uint16 recordNumber, // input parameter - ID of register file or buffer
                          // being accessed
     uint16 index,  // input parameter - byte index into register file or buffer
                    // being accessed
 ...
 *define the two low
 * level abstract functions to write to and read from the SPI the definitions
 *should be in deca_spi.c file.
...
extern int writetospi // returns 0 for success, or, -1 for error.
    (uint16
         headerLength, // input parameter - number of bytes header being written
     const uint8 *headerBuffer, // input parameter - pointer to buffer
                                // containing the 'headerLength' bytes of header
                                // to be written
     uint32 bodylength, // input parameter - number of bytes data being written
     const uint8 *bodyBuffer // input parameter - pointer to buffer containing
                             // the 'bodylength' bytes od data to be written
     );
 ...
 * @param bodyBuffer    - pointer to buffer containing to return the data (NB:
 ...
 */
extern int readfromspi // returns offset where requested data begins in supplied
                       // buffer, or, -1 for error.
    (uint16 headerLength, // input parameter - number of bytes header to write
     ...
     uint8 *readBuffer  // input parameter - pointer to buffer containing to
                        // return the data (NB: size required = headerLength +
                        // readlength)
     );
