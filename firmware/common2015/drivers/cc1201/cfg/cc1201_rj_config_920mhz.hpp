...
// GPIO configurations are [almost] completely independent from modulation
// implementations
#define SMARTRF_SETTING_IOCFG3 0x30  // HIGHZ
#define SMARTRF_SETTING_IOCFG2 \
    0x07  // Asserted when PKT_CRC_OK is set. De-asserted at first byte read
          // from the RX FIFO
#define SMARTRF_SETTING_IOCFG1 \
    0x30  // HIGHZ [used as SO for SPI when CSn is LOW]
#define SMARTRF_SETTING_IOCFG0 0x30  // HIGHZ
//#define SMARTRF_SETTING_IOCFG0             0x06         // Asserted when sync
// word has been received and de-asserted at the end of the packet
//#define SMARTRF_SETTING_IOCFG2             0x0D         // RSSI calculation is
// valid
//#define SMARTRF_SETTING_IOCFG2             0x11         // High if RSSI level
// is above threshold
//#define SMARTRF_SETTING_IOCFG0             0x0B         // Asserted when
// preamble quality threshold is reached
//#define SMARTRF_SETTING_IOCFG0             0x0C         // Asserted when
// preamble is deemed to be valid
...
#define SMARTRF_SETTING_SYNC_CFG1 \
    0x84  // 32 bit sync word length; Threshold value of `10` (or 0x04 here)
          // from TI's SmartRF Studio GUI
#define SMARTRF_SETTING_SYNC_CFG0 \
    0x08  // PQT gating disabled; External sync detect disabled; Strict sync
          // word check disabled
...
#define SMARTRF_SETTING_CHAN_BW \
    0x01  // FROM RFSTUDIO - Decimation factor of 12;
#define SMARTRF_SETTING_MDMCFG2 \
    0x00  // FROM RFSTUDIO - I don't belive this config value matters for us but
          // leaving it here just to be safe
#define SMARTRF_SETTING_MDMCFG1 \
    0x42  // FROM RFSTUDIO - Always search for sync word; FIFO enabled;
          // Manchester mode disabled; Invert data disabled; Collision detect
          // disabled; DVGA gain of -18dB; IQ-channels
...
#define SMARTRF_SETTING_AGC_GAIN_ADJUST \
    0x00  // Write RSSI offset value here for pre-calculated RSSI readings
...
#define SMARTRF_SETTING_FIFO_CFG \
    0xD0  // Automatically flush the last packet received on bad CRC (default);
          // bits 6..0 sets the threshold value for the RX/TX FIFOs - 81 RX
          // bytes & 47 TX bytes
#define SMARTRF_SETTING_DEV_ADDR 0x00  // Device address used for filtering
#define SMARTRF_SETTING_SETTLING_CFG \
    0x0B  // PARTS FROM RFSTUDIO - Calibrate when going from IDLE->RX/TX/FSTXON
          // (default); 70/30 us settling time (default); 60 us synthesizer
          // regulator settling time (default)
#define SMARTRF_SETTING_FS_CFG \
    0x12  // FROM RFSTUDIO - Out of lock detector enabled; LO divider = 4
          // (default)
...
#define SMARTRF_SETTING_PKT_CFG2 \
    0x00  // TX/RX data byte swap disabled (default); Standard packet mode
          // (default); CCA mode always give clear channel indication; Normal
          // FIFO mode (MDMCFG1.FIFO_EN must be set to 1 AND
          // MDMCFG0.TRANSPARENT_MODE_EN must be set to 0)
#define SMARTRF_SETTING_PKT_CFG1 \
    0x03  // FEC disabled (default); Data whitening disabled (default); PN9
          // sequence disabled (default); No address check (default); CRC check
          // in RX & calculate in TX initialized to 0xFFF (default); Append
          // status bytes to RX FIFO enabled (default)
#define SMARTRF_SETTING_PKT_CFG0 \
    0x20  // Variable packet length; UART mode disabled (default); UART swap
          // bits disabled (default)
#define SMARTRF_SETTING_RFEND_CFG1 \
    0x0F  // RXOFF -> IDLE (default); RX timeout disabled; RX timeout qualifier
          // continues RX mode on RX timeout if sync word has been found, or PQT
          // is reached, or CS is asserted
#define SMARTRF_SETTING_RFEND_CFG0 \
    0x30  // wake-up pulses disabled (default); TXOFF mode -> RX (NOT DEFAULT);
          // Termindate on bad packet disabled (default); Direct RX termination
          // & antenna diversity disabled (default)
#define SMARTRF_SETTING_PA_CFG1 \
    0x7F  // PA ramping and ASK/OOK enabled; 14dBm output power
#define SMARTRF_SETTING_PA_CFG0 \
    0x56  // first intermediate power level of 2/16 (default); Second
          // intermediate power level of 13/16 (default); 3 symbol ramp time
          // [CHECK UPSAMPLER_P valid values]
#define SMARTRF_SETTING_ASK_CFG \
    0x0F  // Only used in ASK/OOK modes; Data filter scale bandwidth of 0.28
          // (0x00 - default); ASK/OOK depth of 0x0F (default)
#define SMARTRF_SETTING_PKT_LEN \
    0xFF  // Max allowed packet length in variable mode
#define SMARTRF_SETTING_IF_MIX_CFG 0x00  // Zero-IF configuration
// #define SMARTRF_SETTING_FREQOFF_CFG        0x2B  // FROM RFSTUDIO - THIS IS A
// PRETTY DAMN IMPORTANT CONFIGURATION VALUE FOR 4-GFSK!
...
#define SMARTRF_SETTING_ECG_CFG \
    0x00  // External clock frequency set with a division factor of 64. Probably
          // can ignore
#define SMARTRF_SETTING_EXT_CTRL \
    0x01  // Pin control disabled; External 40kHz clock disabled; Burst address
...
// ===== VALUES BELOW HERE DON'T MATTER FOR INITIAL CONFIGURATION - THEY'RE ALL
