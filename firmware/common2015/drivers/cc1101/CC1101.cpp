...
CC1101::CC1101(PinName mosi, PinName miso, PinName sck, PinName cs,
    ...
    // [X] - 1 - Setup the chip and only signal the base class if device is
    // tested and confirmed to be electrically connected
    if (powerUp()) {
        // [X] - 2a - Don't completely initialize if device fails to
        // successfully start
        // =================
        LOG(FATAL, "CC1101 Failure");
    } else {
        // [X] - 2 - Call the base class method for beginning full class
...
bool CC1101::isConnected() const {
    ...
    // =================
    // Note: This does not necessarily mean the link must be reliable, this only
    // needs to determine: `Can the perheripial provide communication with a 2nd
    // initialized CC1101?`
...
int32_t CC1101::selfTest() {
    // [X] - 1 - Get the chip's version number and fail if different from what
    // was expected.
    _chip_version = status(CCXXX1_VERSION);

    if (_chip_version != CCXXX1_EXPECTED_VERSION_NUMBER) {
        // [X] - 2 - Send message over serial port if version register is not
        // what was expected
        // =================
        LOG(SEVERE, "CC1101",
            "FATAL ERROR\r\n"
            "  Wrong version number returned from chip's 'VERSION' register "
            "(Addr: 0x%02X)\r\n"
            "\r\n"
            "  Expected: 0x%02X\r\n"
            "  Found:    0x%02X\r\n"
            ...
            "    - Check that the chip is fully connected with no soldering "
            "errors\r\n"
...
void CC1101::set_init_vars() {
    ...
    *_cs = 1;
    ...
    // turn off address packet filtering and assign 0 (broadcast address) to the
    // address value
    ...
    // enable automatically flushing the RX buffer on a bad CRC (only works if 1
    // packet is in the RX buffer)
    _pck_control.autoflush_en = true;

    // enable appending 2 status bytes to the end of every packet that includes
    // the CRC and
    ...
    // setup how the payload of the packet is transmitted - default to a fixed
    // length of 61 bytes
    _pck_control.length_type = PACKET_VARIABLE;
    //_pck_control.length_type = PACKET_FIXED;
    _pck_control.size =
        61;  // indicates max packet size when using variable packet types

    // this is a preamble threshold for determining when a packet should be
    // accepted
    _pck_control.preamble_thresh = 2;

    // these values determine how the frequency bands and channels are
    // distributed as well as defining the modulation type
...
void CC1101::power_on_reset() {
    ...
    *_cs = 1;
    ...
    // bring SPI lines to a defined state. Reasons are outlined in CC1101
    ...
    *_cs = 0;
    ...
    delete _spi;
    // wait for the SO line to go low again. Once low, reset is complete and
    // CC1101 is in IDLE state
    ...
    *_cs = 1;
    ...
    // reestablish the SPI bus for the final time after removing the DigitalIn
    // object
    delete SO2;
    setup_spi();
...
void CC1101::init() {
    ...
    uint8_t paTable[] = {0xC0, 0x00, 0x00, 0x00,
...
#if CCXXX1_DEBUG_MODE > 1
    /*  For the debug mode, this will determine how long the CC1101 takes to
...
// =================
// *Note: Takes very few cycles, so might as well wait before returning to
// elimate querky errors.
// while(mode() != 0x0D);
...
int32_t CC1101::getData(uint8_t* buf, uint8_t* length) {
    ...
    if (rx_size & CCXXX1_RXFIFO_MASK) {
        // [X] - 3.1 - Check if the received data will fit in the provided
        // buffer - fill the buffer if so.
        // =================
        if (rx_size <= *length) {
            // [X] - 3.1a - Read in received data from the CC1101 and put the
            // contents in the provided buffer.
            *length = rx_size;  // set the correct length
            read_reg(CCXXX1_RXFIFO, buf, *length);

            /*  The RSSI value takes a bit to be determined by the CC1101, so
               having 2 separate SPI reads gives
            ...
            */
            ...
            read_reg(CCXXX1_RXFIFO, &buf[*length + 1], 2);

            // Update the RSSI reading.
            rssi(status_bytes[0]);
            _lqi = status_bytes[1] &
                   CCXXX1_RXFIFO_MASK;  // MSB of LQI is the CRC_OK bit - The
                                        // interrupt is only triggered if CRC is
                                        // OK, so no need to check again.
            ...
            // [X] - 3.2c - Go back to the receiving state since CC1101 is
            // configured for transitioning to IDLE on receiving a packet.
            rx_mode();
            return 0;  // success
        } else {
            *length = rx_size;
...
uint8_t CC1101::lqi() { return 0x3F - (_lqi & 0x3F); }

// returns the CC1101's VERSION register that specifices what exact chip version
// is being used
uint8_t CC1101::version() { return _chip_version; }
...
// RSSI
int16_t CC1101::rssi() { return _rssi; }
...
uint8_t CC1101::status() { return strobe(CCXXX1_SNOP); }
...
void CC1101::freq(uint32_t freq) {
    /* calculate the value that is written to the register for settings the base
     * frequency
     * that the CC1101 should use for sending/receiving over the air. Default
     * value is equivalent
     * to 901.83 MHz.
     */

    // this is split into 3 bytes that are written to 3 different registers on
    // the CC1101
    uint32_t reg_freq = freq / (CCXXX1_CRYSTAL_FREQUENCY >> 16);

    rfSettings.FREQ2 =
        (reg_freq >> 16) &
        0xFF;  // high byte, bits 7..6 are always 0 for this register
...
void CC1101::datarate(uint32_t rate) {
    ...
    // have to be careful with bit shifting here since it requires a large
    // amount of shifts
    uint32_t shift_val = 28 - (_modem.data_rate_exp & 0x0F);

    // compute the register value and assign it
    rfSettings.MDMCFG3 =
        ((_datarate) / (CCXXX1_CRYSTAL_FREQUENCY >> shift_val)) - 256;
}
...
void CC1101::assign_modem_params() {
    rfSettings.MDMCFG4 = (_modem.channel_bw_exp & 0x03) << 6 |
                         (_modem.channel_bw & 0x03) << 4 |
                         (_modem.data_rate_exp & 0x0F);
    rfSettings.MDMCFG2 =
        _modem.dc_filter_off_en << 7 | (_modem.mod_type & 0x07) << 4 |
        _modem.manchester_encode_en << 3 | (_modem.sync_mode & 0x07);
    rfSettings.MDMCFG1 = _modem.fec_en << 7 |
                         (_modem.preamble_bytes & 0x07) << 4 |
                         (_modem.channel_space_exp & 0x03);
}
// ===============
void CC1101::assign_packet_params() {
    rfSettings.PCKCTRL0 =
        _pck_control.whitening_en << 6 | (_pck_control.format_type & 0x3) << 4 |
        _pck_control.crc_en << 2 | (_pck_control.length_type & 0x3);
    rfSettings.PCKCTRL1 = (_pck_control.preamble_thresh & 0x07) << 5 |
                          _pck_control.autoflush_en << 3 |
                          _pck_control.status_field_en << 2 |
                          (_pck_control.addr_check & 0x03);
...
void CC1101::assign_channel_spacing(uint32_t spacing) {
    ...
    // compute the register value and assign it
    rfSettings.MDMCFG0 =
        (spacing / (CCXXX1_CRYSTAL_FREQUENCY >> shift_val)) - 256;
}
void CC1101::set_rf_settings() {
    ...
    // there can be 16 different channel numbers. The bandwidth and spacing are
    // defined in other registers
    ...
    rfSettings.MCSM2 =
        (RX_TIME_RSSI << 4) | (RX_TIME_QUAL << 3) | (RX_TIME & 0x07);
    ...
    rfSettings.MCSM1 = ((CCA_MODE & 0x03) << 4) | ((RXOFF_MODE & 0x03) << 2) |
                       (TXOFF_MODE & 0x03);
    ...
    rfSettings.MCSM0 = ((FS_AUTOCAL & 0x03) << 4) | ((PO_TIMEOUT & 0x03) << 2) |
                       (PIN_CTRL_EN << 1) | (XOSC_FORCE_ON);

    bool FOC_BS_CS_GATE = false;
    uint8_t FOC_PRE_K = 0x03;
    bool FOC_POST_K = true;
    uint8_t FOC_LIMIT = 0x01;
    rfSettings.FOCCFG = 0x40 | (FOC_BS_CS_GATE << 5) |
                        ((FOC_PRE_K & 0x03) << 3) | (FOC_POST_K << 2) |
                        (FOC_LIMIT & 0x03);
