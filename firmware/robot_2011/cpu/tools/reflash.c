...
// Entry point
// MC_FMR must already be set for flash operations.  USB must be configured, but RX bank doesn't matter.
// There must be no pending USB RX data except for the first packet of flash data
// (this is necessary to allow the current RX bank to be detected).
__attribute__ ((section (".reflash")))
void reflash_main(int total_size)
{
	...
	// there is no way to recover.  We must reach this point soon enough after starting
	// the flash procedure that there is no chance of two packets having arrived.
	while (!((csr = AT91C_UDP_CSR[EP_OUT]) & (AT91C_UDP_RX_DATA_BK0 | AT91C_UDP_RX_DATA_BK1))) wdt_reset();
	...
	for (ptr = start; ptr < start + total_size;)
	{
		...
		if (bank == 0)
		{
			...
			while (!(AT91C_UDP_CSR[EP_OUT] & AT91C_UDP_RX_DATA_BK1)) wdt_reset();
			ptr += reflash_read(ptr);
		...
		// The page size is either 128 or 256 bytes while the max packet size is 64 bytes,
