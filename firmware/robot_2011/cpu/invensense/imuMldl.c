 ...
 **/
tMLError MLDLCfgHardware( unsigned char auxSlaveAddr, 
...
tMLError MLDLDmpStart( void )
{
    ...
    MLDLResetDmp();
    startAddress = ((unsigned short)MPUGetRegisterShadow( MPUREG_DMP_CFG_1 ) << 8) |
 ...
 * @return  Zero if the command is successful, an error code otherwise.
 */
tMLError MLDLPowerMgmtMPU( unsigned char sleep, 
                                 ...
                                 unsigned char disable_gz )
{
    ...
    if(!sleep && sleepChange) {     // waking up from sleep
        MLDLSetRegisterMPU( MPUREG_PWR_MGM, b );
        MLOSSleep(5);
        if(i2cPTStatus) {   // was in master mode (that is, pass-through was disabled)
 ...
 */
unsigned short MLDLGetDMPData(unsigned char src, 
 ...
 *          that changes keys into addresses. This function is set with setGetAddress().
 *
                                 ...
                                 unsigned short length, unsigned char* buffer )
{
    ...
    if( bank != MPUGetRegisterShadow(MPUREG_BANK_SEL) ) {     // only write bank register if it's changed
 ...
 */
static 
tMLError MLDLSetMemoryMPU_one_bank( unsigned char bank, unsigned short memAddr, 
                                          unsigned short length, const unsigned char* buffer )
{
    unsigned char  b;
    tMLError ec;
...
tMLError MLDLGetFifoLength(unsigned short * len)
{
    ...
    result = ReadBurst( mldlData.mpuSlaveAddr, MPUREG_FIFO_COUNTH, 
                        2, fifoBuf );
...
tMLError MLDLReadFifo(unsigned char *data, unsigned short len)
{
    INVENSENSE_FUNC_START
    tMLError result;
    result = ReadBurst( mldlData.mpuSlaveAddr, 
                        MPUREG_FIFO_R_W, 
                        len, 
                        data);
    if (ML_SUCCESS != result) {
