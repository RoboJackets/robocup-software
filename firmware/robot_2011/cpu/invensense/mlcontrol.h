...
#define MLCTRL_GRID_CALLBACK_DEFAULT             0
    ...
    typedef struct {
        unsigned short sensitivity[4];         // Sensitivity of control signal 1, 2, 3, and 4.
        unsigned short functions;              // Indicates what functions will be used. Can be a bitwise OR of ML_GRID, ML_SMOOT, ML_DEAD_ZONE, and ML_HYSTERISIS.
        unsigned short controlSignals;         // Used to register which control signals will be processed. Can be a control signal or bitwise OR of multiple control signals.
        unsigned short parameterArray[4];      // Indicates which parameter array is being assigned to a control signal. Must be one of ML_GYROS, ML_ANGULAR_VELOCITY.
        unsigned short parameterAxis[4];       // Indicates which axis of the parameter array will be used. Must be ML_ROLL, ML_PITCH, or ML_YAW.
        int gridThreshold[4];                  // Threshold of the control signal at which the grid number will be incremented or decremented.
        int gridMaximum[4];                    // Maximum grid number for the control signal.
        void (*gridCallback)(                  // User defined callback that will trigger when the grid location changes.
            unsigned short controlSignal,      // Indicates which control signal crossed a grid threshold. Must be one of ML_CONTROL_1, ML_CONTROL_2, ML_CONTROL_3 or ML_CONTROL_4.
            int *gridNum,                      // An array of four numbers representing the grid number for each control signal.
            int *gridChange                    // An array of four numbers representing the change in grid number for each control signal.
        );
