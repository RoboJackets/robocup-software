...
/**
 * @brief A class for interfacing with the kicker board, which is based on an AVR chip.
 */
...
public:
     ...
     * @param progFilename Path to kicker program binary file that will be loaded by the flash() method
     */
    KickerBoard(
        PinName mosi,
        PinName miso, 
    ...
    /**
     * @brief Reflashes the program on the kicker board MCU with the file specified in the constructor.
     *
     * @param onlyIfDifferent If true, compares the program file to the MCU's flash and only goes
...
protected:
    /**
     * @brief Uses the given function to check if it's return value equals the expected value.
     *
     * @return True if the return value was the expected value, false otherwise
     */
    bool verify_param(const char *name, int expected, int (AVR910::*paramMethod)(), bool verbose = false);
