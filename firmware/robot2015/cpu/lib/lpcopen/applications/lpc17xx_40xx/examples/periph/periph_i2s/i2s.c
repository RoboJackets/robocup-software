...
/** @defgroup EXAMPLES_PERIPH_17XX40XX_I2S LPC17xx/40xx I2S example
...
static void App_Interrupt_Test(void)
{
	...
	Chip_I2S_Int_Cmd(LPC_I2S, I2S_RX_MODE,    ENABLE, 4);
	...
	while (continue_Flag) {
		...
		case 'x':
			continue_Flag = 0;
			Chip_I2S_Int_Cmd(LPC_I2S, I2S_RX_MODE,    DISABLE, 4);
...
static void App_Polling_Test(void)
{
	...
	while (continue_Flag) {
		...
		if ((Chip_I2S_GetLevel(LPC_I2S, I2S_TX_MODE) < 4) && (send_flag == 1)) {
...
static void App_DMA_Test(void)
{
	...
	Chip_I2S_DMA_Cmd(LPC_I2S, I2S_TX_MODE, I2S_DMA_REQUEST_NUMBER_1, ENABLE, 4);
	Chip_I2S_DMA_Cmd(LPC_I2S, I2S_RX_MODE, I2S_DMA_REQUEST_NUMBER_2, ENABLE, 4);
	...
	dmaChannelNum_I2S_Rx = Chip_DMA_GetFreeChannel(LPC_GPDMA, GPDMA_CONN_I2S_Channel_1);
	...
	while (continue_Flag) {
		...
		case 'x':
			continue_Flag = 0;
			Chip_I2S_DMA_Cmd(LPC_I2S, I2S_RX_MODE, 2, DISABLE, 1);
			Chip_I2S_DMA_Cmd(LPC_I2S, I2S_TX_MODE, 1, DISABLE, 1);
...
{
	while ((ring_buff_get_status(&ring_buffer) != BUFFER_FULL) && (Chip_I2S_GetLevel(LPC_I2S, I2S_RX_MODE) > 0)) {
		ring_buffer.buffer[ring_buffer.write_index++] = Chip_I2S_Receive(LPC_I2S);
	}
	while ((ring_buff_get_status(&ring_buffer) != BUFFER_EMPTY) && (Chip_I2S_GetLevel(LPC_I2S, I2S_TX_MODE) < 8)) {
...
int main(void)
{
	...
	Chip_I2S_Config(LPC_I2S, I2S_RX_MODE, &audio_Confg);
