...
/*
 * 512 Bytes of system stack is more than sufficient for most of the applications
 * because all the application stack is allocated in heap. But in case any application
 * wants to have more default stack it can override the default size by defining STACK_SIZE
...
/*
 * Heap over flow function, the default function in redlib library checks
 * the new_end_of_heap against the current stack pointer, when using RTOS
 * the current stack pointer will point to a memory allocated in heap hence
 * the comparison always will fail
 */
unsigned int __check_heap_overflow (void * new_end_of_heap)
{
	return (new_end_of_heap >= (unsigned int)&_vStackTop - STACK_SIZE);
}
