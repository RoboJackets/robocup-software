...
void CommModule::send(RTP_t& packet)
{
    ...
    if (std::binary_search(_open_ports->begin(), _open_ports->end(), packet.port)) {
        ...
        log(WARN, "CommModule", "Failed to send %u byte packet: There is no open socket for port %u", packet.total_size, packet.port);
    }
}
...
void CommModule::receive(RTP_t& packet)
{
    ...
    if (std::binary_search(_open_ports->begin(), _open_ports->end(), packet.port)) {
        ...
        log(WARN, "CommModule", "Failed to receive %u byte packet: There is no open socket for port %u", packet.total_size, packet.port);
    }
}
