 ...
 *
 *        The internal Flash memory access is described in the LPC1768 and
 * LPC11U24 usermanual.
 ...
 *        revision 1.1  12-Mar-2010   chaged: to make possible to reserve flash
 * area for user
 *                                            it can be set by
 * USER_FLASH_AREA_START and USER_FLASH_AREA_SIZE in IAP.h
 *        revision 2.0  26-Nov-2012   LPC11U24 code added
 *        revision 2.1  26-Nov-2012   EEPROM access code imported from Suga
 * koubou san's (http://mbed.org/users/okini3939/) library
 ...
 *        revision 3.1.1 16-Jan-2015  Target MCU name changed for better
...
#define TARGET_SECTOR \
    (29)  //  use sector 29 as target sector if it is on LPC1768
 ...
 *        Table 568 "Sectors in a LPC17xx device", Section 5. "Sector numbers",
 * usermanual
 *
...
constexpr char* sector_start_adress[] = {
    (char*)FLASH_SECTOR_0,  (char*)FLASH_SECTOR_1,  (char*)FLASH_SECTOR_2,
    (char*)FLASH_SECTOR_3,  (char*)FLASH_SECTOR_4,  (char*)FLASH_SECTOR_5,
    (char*)FLASH_SECTOR_6,  (char*)FLASH_SECTOR_7,  (char*)FLASH_SECTOR_8,
    (char*)FLASH_SECTOR_9,  (char*)FLASH_SECTOR_10, (char*)FLASH_SECTOR_11,
    (char*)FLASH_SECTOR_12, (char*)FLASH_SECTOR_13, (char*)FLASH_SECTOR_14,
    (char*)FLASH_SECTOR_15, (char*)FLASH_SECTOR_16, (char*)FLASH_SECTOR_17,
    (char*)FLASH_SECTOR_18, (char*)FLASH_SECTOR_19, (char*)FLASH_SECTOR_20,
    (char*)FLASH_SECTOR_21, (char*)FLASH_SECTOR_22, (char*)FLASH_SECTOR_23,
    (char*)FLASH_SECTOR_24, (char*)FLASH_SECTOR_25, (char*)FLASH_SECTOR_26,
    (char*)FLASH_SECTOR_27, (char*)FLASH_SECTOR_28, (char*)FLASH_SECTOR_29};
 ...
 *        Table 334 "LPC11U1x/2x flash sectors", Section 20. "Sector numbers",
 * usermanual
 *
 ...
 *
 *        Table 588 "ISP Return Codes Summary", Section 7.15 "ISP Return Codes",
 * usermanual
 */
...
public:
     ...
     */
    IAP()
        : iap_entry(reinterpret_cast<IAP_call>(IAP_LOCATION)),
     ...
     *  @param    end      an End Sector Number (should be greater than or equal
     * to start sector number).
     *  @return error code: CMD_SUCCESS | BUSY | SECTOR_NOT_BLANK |
     * INVALID_SECTOR
     */
     ...
     *  @param    end      an End Sector Number (should be greater than or equal
     * to start sector number).
     *  @return   error code: CMD_SUCCESS | BUSY |
    ...
    /** Copy RAM to Flash
     *
     *  @param    source_addr    Source RAM address from which data bytes are to
     * be read. This address should be a word boundary.
     *  @param    target_addr    Destination flash address where data bytes are
     * to be written. This address should be a 256 byte boundary.
     *  @param    size           Number of bytes to be written. Should be 256 |
     * 512 | 1024 | 4096.
     *  @return   error code: CMD_SUCCESS | SRC_ADDR_ERROR (Address not a word
     * boundary) | DST_ADDR_ERROR (Address not on correct boundary) |
     * SRC_ADDR_NOT_MAPPED | DST_ADDR_NOT_MAPPED | COUNT_ERROR (Byte count is
     * not 256 | 512 | 1024 | 4096) | SECTOR_NOT_PREPARED_FOR_WRITE_OPERATION |
     * BUSY
     */
     ...
     *
     *  @param    source_addr Starting flash or RAM address of data bytes to be
     * compared. This address should be a word boundary.
     *  @param    target_addr Starting flash or RAM address of data bytes to be
     * compared. This address should be a word boundary.
     *  @param    size         Number of bytes to be compared; should be a
     * multiple of 4.
     *  @return   error code: CMD_SUCCESS | COMPARE_ERROR | COUNT_ERROR (Byte
     * count is not a multiple of 4) | ADDR_ERROR | ADDR_NOT_MAPPED
     */
     ...
     *  @param    source_addr    Source RAM address from which data bytes are to
     * be read.
     *  @param    target_addr    Destination EEPROM address where data bytes are
     * to be written.
     *  @param    size           Number of bytes to be written.
     *  @return   error code: CMD_SUCCESS | SRC_ADDR_NOT_MAPPED |
     * DST_ADDR_NOT_MAPPED
     *  Remark: The top 64 bytes of the EEPROM memory are reserved and cannot be
     * written to.
     */
     ...
     *
     *  @param    source_addr    Source EEPROM address from which data bytes are
     * to be read.
     *  @param    target_addr    Destination RAM address where data bytes are to
     * be written.
     *  @param    size           Number of bytes to be written.
     *  @return   error code: CMD_SUCCESS | SRC_ADDR_NOT_MAPPED |
     * DST_ADDR_NOT_MAPPED
     ...
     *  @param    end      End page number (should be greater than or equal to
     * start page).
     *  @return   error code: CMD_SUCCESS | BUSY |
     * SECTOR_NOT_PREPARED_FOR_WRITE_OPERATION | INVALID_SECTOR
     */
