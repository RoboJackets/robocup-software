...
#pragma message("Starting memory address for storage: " TO_STRING(       \
    USER_FLASH_AREA_START) "\r\nNumber of allocated bytes:\t" TO_STRING( \
    USER_FLASH_AREA_SIZE))
unsigned char user_area[USER_FLASH_AREA_SIZE]
    __attribute__((section(".ARM.__at_" TO_STRING(USER_FLASH_AREA_START))));
...
// unsigned char user_area[ size ] __attribute__((section(".ARM.__at_0x78000"),
// zero_init));
...
int IAP::read_ID(void) {
    ...
    __enable_irq();
    //  return ( (int)IAP_result[ 0 ] );
    return ((int)IAP_result[1]);  //  to return the number itself (this command
                                  //  always returns CMD_SUCCESS)
}
...
int* IAP::read_serial(void) {
    ...
    __enable_irq();
    //  return ( (int)IAP_result[ 0 ] );
    return ((int*)&IAP_result[1]);  //  to return the number itself (this
                                    //  command always returns CMD_SUCCESS)
}
...
int IAP::blank_check(int start, int end) {
    ...
    IAP_command[2] = (unsigned int)end;    //  End Sector Number (should be
...
int IAP::write(char* source_addr, char* target_addr, int size) {
    ...
    IAP_command[1] =
        (unsigned int)target_addr;  //  Destination flash address where data
                                    //  bytes are to be written. This address
                                    //  should be a 256 byte boundary.
    IAP_command[2] = (unsigned int)source_addr;  //  Source RAM address from
                                                 //  which data bytes are to be
                                                 //  read. This address should
                                                 //  be a word boundary.
    IAP_command[3] = size;  //  Number of bytes to be written. Should be 256 |
                            //  512 | 1024 | 4096.
...
int IAP::compare(char* source_addr, char* target_addr, int size) {
    ...
    IAP_command[1] = (unsigned int)target_addr;  //  Starting flash or RAM
                                                 //  address of data bytes to be
                                                 //  compared. This address
                                                 //  should be a word boundary.
    IAP_command[2] = (unsigned int)source_addr;  //  Starting flash or RAM
                                                 //  address of data bytes to be
                                                 //  compared. This address
                                                 //  should be a word boundary.
    IAP_command[3] =
...
int IAP::reserved_flash_area_size(void) { return (USER_FLASH_AREA_SIZE); }
...
int IAP::write_eeprom(char* source_addr, char* target_addr, int size) {
    ...
    IAP_command[1] =
        (unsigned int)target_addr;  //  Destination EEPROM address where data
                                    //  bytes are to be written. This address
                                    //  should be a 256 byte boundary.
    IAP_command[2] = (unsigned int)source_addr;  //  Source RAM address from
                                                 //  which data bytes are to be
...
int IAP::read_eeprom(char* source_addr, char* target_addr, int size) {
    ...
    IAP_command[1] = (unsigned int)source_addr;  //  Source EEPROM address from
                                                 //  which data bytes are to be
                                                 //  read. This address should
                                                 //  be a word boundary.
    IAP_command[2] =
        (unsigned int)target_addr;  //  Destination RAM address where data bytes
                                    //  are to be written. This address should
                                    //  be a 256 byte boundary.
    IAP_command[3] = size;  //  Number of bytes to be written. Should be 256 |
                            //  512 | 1024 | 4096.
    IAP_command[4] = cclk_kHz;  //  CPU Clock Frequency (CCLK) in kHz.
    __disable_irq();
