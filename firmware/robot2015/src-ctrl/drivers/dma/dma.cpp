...
bool DMA::Init() {
    ...
    // reset_periph_modes();
    // set_periph_mode(1);  // This sets it up for use with Timer0 match0 as
    // being the selection
    ...
    LOG(INF2, "DMA NVIC vector set to address 0x%08X",
...
uint8_t DMA::find_channel() {
    ...
    for (channel = 0; channel < DMA_NUM_CHANNELS; channel++) {
        ...
        if (channel_taken == false) {
            chan = (LPC_GPDMACH_TypeDef*)((uint32_t)_LPC_DMA_CHAN0 +
                                          (channel *
                                           sizeof(LPC_GPDMACH_TypeDef*)));
            return channel;
        }
    }
...
uint8_t DMA::disable_controller() {
    ...
    if (_LPC_DMA_ID->DMACEnbldChns & 0xFF) disable_channels();
...
void DMA::clear_errors() {
    for (int i = 0; i < DMA_NUM_CHANNELS; i++) clear_error(i);
}
 ...
 */
void DMA::set_periph_mode(uint8_t mode) { LPC_SC->DMAREQSEL |= (1 << mode); }

/**
 * [DMA::reset_periph_modes description]
 */
void DMA::reset_periph_modes() { LPC_SC->DMAREQSEL = 0x00; }
