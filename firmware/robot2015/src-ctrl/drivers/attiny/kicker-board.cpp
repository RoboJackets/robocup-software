...
bool KickerBoard::verify_param(const char* name, int expected,
                               int (AVR910::*paramMethod)(), bool verbose) {
    ...
    if (verbose) {
        if (success)
            ...
            printf("Got unexpected value: 0x%X", val);
    }
...
int KickerBoard::flash(bool onlyIfDifferent, bool verbose) {
    ...
    if (!verify_param("Vendor ID", ATMEL_VENDOR_CODE, &AVR910::vendor, verbose))
        return -2;

    //  Read part family and flash size - see datasheet for code meaning.
    if (verbose) printf("Reading Type ID...");
    ...
    if (!verify_param("Device ID", CHIP_TYPE_ATTINY13A, &AVR910::partnum,
                      verbose))
        return -4;
    ...
    if (fp == NULL) {
        ...
        if (onlyIfDifferent &&
            (verify(fp, ATTINY13A_PAGE_SIZE, ATTINY13A_NUM_PAGES) == 0))
            shouldProgram = false;
        ...
        if (!shouldProgram) {
            ...
            bool nSuccess =
                program(fp, ATTINY13A_PAGE_SIZE, ATTINY13A_NUM_PAGES);
    ...
    if (verbose)
        printf(
            "toggling reset line to exit programming mode and reset kicker "
            "board\r\n");
    _nReset = 0;
    wait_ms(20);
    _nReset = 1;
