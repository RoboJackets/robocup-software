...
void Task_SerialConsole(void const* args) {
    ...
    // tasks
    os_status = osThreadSetPriority(threadID, osPriorityLow);
    ...
    // Reset our priorty
    os_status = osThreadSetPriority(threadID, threadPriority);
    ASSERT(os_status == osOK);
    ...
    while (true) {
        ...
        executeIterativeCommand(std::cout);
        ...
        if (Console::CommandReady() == true) {
            ...
            os_status = osThreadSetPriority(threadID, osPriorityAboveNormal);
            ASSERT(os_status == osOK);

            // Disable UART interrupts & execute the command
            NVIC_DisableIRQ(UART0_IRQn);
            executeLine(std::cout, Console::rxBufferPtr());
