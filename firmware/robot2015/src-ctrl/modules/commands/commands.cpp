...
int cmd_alias(cmd_args_t &args) {
  ...
  if (args.empty()) {
    for (uint8_t i = 0; i < commands.size(); i++) {
      ...
      while (a < commands[i].aliases.size() && commands[i].aliases[a] != "\0") {
...
int cmd_info(cmd_args_t &args) {
  if (!args.empty()) {
    ...
    printf("\tBase ID:\t");
    if (ds2411_read_id(RJ_BASE_ID, &id) != ID_CRC_MATCH) {
  ...
  if (args.empty() || args.size() > 1) {
    ...
    if (str_baud == "--list" || str_baud == "-l") {
      ...
      if (std::find(valid_rates.begin(), valid_rates.end(), new_rate) !=
        ...
        printf("%u is not a valid baudrate. Use \"--list\" to show valid "
               "baudrates.\r\n",
               new_rate);
      }
    } else {
    ...
    if (args[0] == "bright") {
      if (args.size() > 1) {
        ...
        if (bri > 0 && bri <= 1.0) {
          ...
          printf("Invalid brightness level of %.2f. Use 'state' command "
    ...
    */
    for (unsigned int i = 0; i < 15; i++) {
      ...
      if (p != nullptr) {
        ...
        printf("%-4u\t  %-3u\t %s\t%-6u\t   %-7u  %-7u  "
               "%-7u (%.0f%%|%-.0f%%)\r\n",
               p->task_id, p->prio, state_now, p->delta_time, max_used, alloc,
               now, util_now * 100, util_max * 100);
...
int cmd_radio(cmd_args_t &args) {
  ...
  if (args.size() == 1 || args.size() == 2) {
    ...
    if (args.size() > 1)
      portNbr = (rtp::Port)atoi(args[1].c_str());
    ...
    if (args[0] == "show") {
      ...
      printf("Placing %u, %u byte packet(s) in TX buffer with ACK set.\r\n", i,
             pck.payload.size());
    ...
    if (args[0] == "stress-test") {
      ...
      printf("Beginning radio stress test with %u %u byte "
      ...
      printf("Stress test finished in %.1fms.\r\n",
             (clock() - start_tick) / static_cast<double>(CLOCKS_PER_SEC) *
                 1000);
    }
  } else {
    show_invalid_args(args);
...
int cmd_pong(cmd_args_t &args) {
  ...
  while (true) {
    ...
    if (Console::Instance()->pc.readable())
      break;
  }
...
int cmd_ping(cmd_args_t &args) {
  ...
  while (true) {
    ...
    if ((clock() - lastPingTime) / CLOCKS_PER_SEC > PingInterval) {
      ...
      CommModule::Instance->send(rtp::packet({pingCount}, rtp::Port::PING));
