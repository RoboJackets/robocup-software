...
WorldModel::~WorldModel() { delete _ballModel; }

void WorldModel::run(bool blueTeam,
                     const std::vector<const SSL_DetectionFrame*>& rawVision) {
    ...
    // Ball sensor
    // FIXME - Detect broken ball sensors and spurious triggers (from spinning
    // or collision)
    for (OurRobot* robot : _state->self) {
    ...
    // add observations to the ball based on ball sensors and filtered robot
    // positions
...
// if a robot has the ball, we need to make an observation
    ...
    bool ballValid = _ballModel->valid(
        curTime);  // checks whether should use prediction or last frame
...
void WorldModel::addRobotObseration(const SSL_DetectionRobot& obs,
                                    uint64_t timestamp, RobotVector& players) {
    ...
    for (RobotModel::shared& model : players) {
        if (model && model->shell() == obs_shell) {
            ...
            model->observation(timestamp, pos,
                               obs.orientation() * RadiansToDegrees);
            return;
        }
    }

    // find an open slot - assumed that invalid models will have been removed
    // already
    for (RobotModel::shared& model : players) {
        if (!model) {
            model =
                RobotModel::shared(new RobotModel(&_robotConfig, obs_shell));
            Geometry2d::Point pos(obs.x() / 1000.0f, obs.y() / 1000.0f);
            model->observation(timestamp, pos,
                               obs.orientation() * RadiansToDegrees);
            return;
...
void WorldModel::updateRobots(vector<RobotModel::shared>& players,
                              uint64_t cur_time) {
    for (RobotModel::shared& model : players) {
