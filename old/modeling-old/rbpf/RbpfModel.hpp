...
class RbpfModel {
public:
    // robotMap is a set of robots for calculating kicks/deflections in some
    // models
    ...
    // dt: change in time
    void predict(rbpf::VectorNd& X, rbpf::MatrixNNd& P, const rbpf::VectorMd& U,
                 double dt) const;
    ...
    // dt: change in time
    // Updates Yhat and S
    virtual void update(rbpf::VectorNd& X, rbpf::MatrixNNd& P,
                        const rbpf::VectorSd& Z, double dt);
...
protected:
    ...
    //   U: control input (m x 1)
    //   dt: change in time
    virtual void transitionModel(rbpf::VectorNd& X, const rbpf::VectorMd& U,
                                 double dt) const = 0;
    ...
    virtual void observationModel(const rbpf::VectorNd& X,
                                  rbpf::VectorSd& out) const = 0;
    ...
    Modeling::RobotModel::RobotMap*
        _robotMap;  // set of robots for kicks/deflections
