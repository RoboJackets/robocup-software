...
// initializes: F, H, Q, R
RbpfModelRolling::RbpfModelRolling(Modeling::RobotModel::RobotMap* _robotMap,
                                   Configuration* config)
    : RbpfModel(_robotMap),
      _processNoiseSqrdPos(config->createDouble(
          "rbpfModelBallRolling/Process Noise Position", 0.2)),
      _processNoiseSqrdVel(config->createDouble(
          "rbpfModelBallRolling/Process Noise Velocity", 1.0)),
      _processNoiseSqrdAcc(config->createDouble(
          "rbpfModelBallRolling/Process Noise Acceleration", 1000.0)),
      _measurementNoiseSqrd(config->createDouble(
          "rbpfModelBallRolling/Measurement Noise Position", 0.01)) {
...
void RbpfModelRolling::transitionModel(VectorNd& X, const VectorMd& U,
                                       double dt) const {
