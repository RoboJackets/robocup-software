...
REGISTER_PLAY_CATEGORY(Gameplay::Plays::OpenField221, "Playing")

namespace Gameplay
{
	namespace Plays
	{
		REGISTER_CONFIGURABLE(OpenField221)
	}
}

ConfigDouble *Gameplay::Plays::OpenField221::_offense_hysteresis;
ConfigDouble *Gameplay::Plays::OpenField221::_support_backoff_thresh;
ConfigDouble *Gameplay::Plays::OpenField221::_mark_hysteresis_coeff;
ConfigDouble *Gameplay::Plays::OpenField221::_support_avoid_teammate_radius;
ConfigDouble *Gameplay::Plays::OpenField221::_support_avoid_shot;
ConfigDouble *Gameplay::Plays::OpenField221::_offense_support_ratio;
ConfigDouble *Gameplay::Plays::OpenField221::_defense_support_ratio;

void Gameplay::Plays::OpenField221::createConfiguration(Configuration *cfg)
{
	_offense_hysteresis = new ConfigDouble(cfg, "OpenField221/Hystersis Coeff", 0.50);
	_support_backoff_thresh = new ConfigDouble(cfg, "OpenField221/Support Backoff Dist", 1.5);
	_mark_hysteresis_coeff = new ConfigDouble(cfg, "OpenField221/Mark Hystersis Coeff", 0.9);
	_support_avoid_teammate_radius = new ConfigDouble(cfg, "OpenField221/Support Avoid Teammate Dist", 0.5);
	_support_avoid_shot = new ConfigDouble(cfg, "OpenField221/Support Avoid Shot", 0.2);
	_offense_support_ratio = new ConfigDouble(cfg, "OpenField221/Offense Support Ratio", 0.7);
	_defense_support_ratio = new ConfigDouble(cfg, "OpenField221/Defense Support Ratio", 0.9);
}
...
float Gameplay::Plays::OpenField221::score ( Gameplay::GameplayModule* gameplay )
{
...
bool Gameplay::Plays::OpenField221::run()
{
	// handle assignments
	...
	if (_striker.robot) {
		...
		if (_striker.robot->pos.nearPoint(ballProj, *_support_avoid_shot)) {
			...
			if ( _support1.robot ) _support1.robot->localObstacles(std::shared_ptr<Obstacle>(new PolygonObstacle(shot_obs)));
			if ( _support2.robot ) _support2.robot->localObstacles(std::shared_ptr<Obstacle>(new PolygonObstacle(shot_obs)));
		}
	}
