...
Geometry2d::Point evaluateBezierVelocity(
    float t, const std::vector<Geometry2d::Point>& controls,
    const std::vector<float>& coeffs) {
    int n = controls.size();
    float j = 1.0 - t;
    Point pt;
    for (int k = 0; k < n; ++k) {
        pt += controls.at(k) * -1 * (n - 1 - k) * pow(j, n - 1 - k - 1) *
              pow(t, k) * coeffs.at(k);
        pt += controls.at(k) * pow(j, n - 1 - k) * k * pow(t, k - 1) *
              coeffs.at(k);
    }
    return pt;
}
...
int binomialCoefficient(int n, int k) {
    if (k > n)
        throw invalid_argument("K greater than N in binomialCoefficient()!");
    if (k == n || k == 0) return 1;
