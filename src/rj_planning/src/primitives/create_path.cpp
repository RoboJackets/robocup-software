...
Trajectory rrt(const LinearMotionInstant& start, const LinearMotionInstant& goal,
    ...
    for (int i = 0; i < kAttemptsToAvoidDynamics; i++) {
        std::vector<Point> points =
            generate_rrt(start.position, goal.position, obstacles, bias_waypoints, config);
...
Trajectory intermediate(const LinearMotionInstant& start, const LinearMotionInstant& goal,
    ...
    for (int i = 0; i < config.intermediate.num_intermediates; i++) {
        ...
        for (double t = config.intermediate.step_size; t < final_inter.dist_to(start.position);
             t += config.intermediate.step_size) {
...
std::vector<rj_geometry::Point> get_intermediates(const LinearMotionInstant& start,
    ...
    std::uniform_real_distribution<> scale_dist(config.intermediate.min_scale,
                                                config.intermediate.max_scale);

    double angle_range = config.intermediate.max_angle - config.intermediate.min_angle;
    ...
    for (int i = 0; i < config.intermediate.num_intermediates; i++) {
        ...
        angle += std::copysign(config.intermediate.min_angle, angle);
