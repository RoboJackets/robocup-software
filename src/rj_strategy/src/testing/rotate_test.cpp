#include "rj_strategy/testing/rotate_test.hpp"

namespace strategy {
using RobotMove = rj_msgs::action::RobotMove;
using GoalHandleRobotMove = rclcpp_action::ClientGoalHandle<RobotMove>;

RotateTest::RotateTest() : RotateTest(0) {}

RotateTest::RotateTest(int r_id)
    : rclcpp::Node(::fmt::format("agent_{}_rotate_node", r_id),
                   rclcpp::NodeOptions{}
                       .automatically_declare_parameters_from_overrides(true)
                       .allow_undeclared_parameters(true)),
      current_position_{std::make_unique<Rotate>(r_id, true, true)},
      robot_id_{r_id} {
    ...
    get_task_timer_ = create_wall_timer(std::chrono::milliseconds(1000 / hz),
                                        std::bind(&RotateTest::get_task, this));
}

void RotateTest::world_state_callback(const rj_msgs::msg::WorldState::SharedPtr& msg) {
...
void RotateTest::alive_robots_callback(const rj_msgs::msg::AliveRobots::SharedPtr& msg) {
    alive_robots_ = msg->alive_robots;
    current_position_->update_alive_robots(alive_robots_);
}

void RotateTest::game_settings_callback(const rj_msgs::msg::GameSettings::SharedPtr& msg) {
    is_simulated_ = msg->simulation;
}

bool RotateTest::check_robot_alive(uint8_t robot_id) {
    if (!is_simulated_) {
...
[[nodiscard]] WorldState* RotateTest::world_state() {
    auto lock = std::lock_guard(world_state_mutex_);
    return &last_world_state_;
}

void RotateTest::goal_response_callback(GoalHandleRobotMove::SharedPtr goal_handle) {
    if (!goal_handle) {
        current_position_->set_goal_canceled();
    }
}

void RotateTest::feedback_callback(
...
void RotateTest::result_callback(const GoalHandleRobotMove::WrappedResult& result) {
    switch (result.code) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 6; i++) {
        auto agent = std::make_shared<strategy::RotateTest>(i);
