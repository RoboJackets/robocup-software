...
std::optional<RobotIntent> PenaltyPlayer::state_to_task(RobotIntent intent) {
    switch (latest_state_) {
        ...
        case SMALL_KICK: {  // less of a kick, more of a "follow" ball closely
            ...
            // The point of making a 1.0 kick speed is to fake dribble since we cannot get the
            // vaccum behavior to work
...
std::string PenaltyPlayer::get_current_state() {
    return position_name_ + std::to_string(static_cast<int>(latest_state_));
}
