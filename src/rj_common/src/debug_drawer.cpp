...
void DebugDrawer::draw_polygon(const rj_geometry::Point* pts, int n, const QColor& qc,
                               const QString& layer) {
    DebugPath dbg;
    dbg.layer_ = find_debug_layer(layer);
    dbg.color_ = color(qc);
    dbg.points_.reserve(n);
    for (int i = 0; i < n; ++i) {
        dbg.points_.push_back(pts[i]);
    }
...
void DebugDrawer::draw_arc(const rj_geometry::Arc& arc, const QColor& qc, const QString& layer) {
    DebugArc dbg;
    dbg.layer_ = find_debug_layer(layer);
    dbg.center_ = arc.center();
    dbg.radius_ = static_cast<float>(arc.radius());
    dbg.start_ = static_cast<float>(arc.start());
    dbg.end_ = static_cast<float>(arc.end());
    dbg.color_ = color(qc);
...
void DebugDrawer::draw_line(const rj_geometry::Segment& line, const QColor& qc,
    ...
    DebugPath dbg;
    dbg.layer_ = find_debug_layer(layer);
    dbg.points_.push_back(line.pt[0]);
    dbg.points_.push_back(line.pt[1]);
    dbg.color_ = color(qc);
