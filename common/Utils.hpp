...
static inline bool ballIsInGoalieBox(Geometry2d::Point point) {
	if (abs(point.x) < Field_Dimensions.Field_GoalFlat / 2.0f) // Ball is in center (rectangular) portion of defensive bubble
	{
		if (point.y > 0 && point.y < Field_Dimensions.Field_ArcRadius) {
			return true;
		} else {
			return false;
		}
	} else if (abs(point.x) < (Field_Dimensions.Field_ArcRadius + Field_Dimensions.Field_GoalFlat / 2.0f)) // Ball is in one of the side (arc) portions of defensive bubble
	{
		double adjusted_x = abs(point.x) - (Field_Dimensions.Field_GoalFlat / 2.0f);
		double max_y = sqrt( (Field_Dimensions.Field_ArcRadius * Field_Dimensions.Field_ArcRadius) - (adjusted_x * adjusted_x));
...
static Geometry2d::Point fromOursToTheirs(Geometry2d::Point &pt) {
	Geometry2d::Point c;
	c.y = Field_Dimensions.Field_Length - pt.y;
	c.x = -pt.x;
