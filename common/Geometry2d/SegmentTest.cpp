...
TEST(Segment, intersects) {
    ...
    ASSERT_FALSE(
        test.intersects(Segment(Point(-14.5, 5), Point(-10, 4)), &temp));
    ASSERT_FALSE(test.intersects(
        Segment(Point(-14.5, 5), Point(6.3635, 0.36367)), &temp));
    ASSERT_TRUE(test.intersects(Segment(Point(-14.5, 5), Point(6.37, 0.362226)),
                                &temp));
    ASSERT_TRUE(Point(6.36364, 0.36364).nearlyEquals(temp));

    ASSERT_TRUE(test.intersects(
        Segment(Point(6.3635, 0.36367), Point(6.37, 0.362226)), &temp));
    ASSERT_TRUE(Point(6.36364, 0.36364).nearlyEquals(temp));

    ASSERT_FALSE(test.intersects(
        Segment(Point(6.37, 0.362226), Point(1000, -220.444)), &temp));

    ASSERT_TRUE(Segment(Point(6.3635, 0.36367), Point(6.37, 0.362226))
                    .intersects(test, &temp));
    ASSERT_TRUE(Point(6.36364, 0.36364).nearlyEquals(temp));

    ASSERT_FALSE(Segment(Point(6.37, 0.362226), Point(1000, -220.444))
                     .intersects(test, &temp));
}
...
TEST(Segment, nearestPoint) {
    ...
    ASSERT_TRUE(Point(10, 4).nearlyEquals(test.nearestPoint(Point(13, 7))))
        << test.nearestPoint(Point(13, 7));
}
