...
const _displayItem = (item, searchTerms, highlightTerms) => {
    ...
    if (descr) {
        ...
        fetch(requestUrl).then((responseData) => responseData.text()).then((data) => {
...
const Search = {
    ...
    htmlToText: (htmlString, anchor) => {
        ...
        if (anchor) {
            ...
            console.warn(
                `Anchored content block not found. Sphinx search tries to obtain it via DOM query '[role=main] ${
                    anchor}'. Check your theme or template.`);
        }
    ...
    _performSearch: (query, searchTerms, excludedTerms, highlightTerms, objectTerms) => {
        ...
        for (const [title, foundTitles] of Object.entries(allTitles)) {
            if (title.toLowerCase().trim().includes(queryLower) &&
                ...
                for (const [file, id] of foundTitles) {
                    ...
                    const boost =
                        titles[file] === title ? 1 : 0;  // add a boost for document titles
                    normalResults.push([
                        docNames[file],
        ...
        // lookup as object
        objectTerms.forEach(
            (term) => normalResults.push(...Search.performObjectSearch(term, objectTerms)));
    ...
    query: (query) => {
        ...
        const results = Search._performSearch(
            searchQuery, searchTerms, excludedTerms, highlightTerms, objectTerms);
    ...
    performObjectSearch: (object, objectTerms) => {
        ...
        const objectSearchCallback = (prefix, match) => {
            const name = match[4] const fullname = (prefix ? prefix + '.' : '') + name;
        ...
        Object.keys(objects).forEach(
            (prefix) => objects[prefix].forEach((array) => objectSearchCallback(prefix, array)));
        return results;
    },
