...
var Sizzle =
    ...
    (function(window) {
        var i, support, Expr, getText, isXML, tokenize, compile, select, outermostContext,
            ...
            // Instance methods
            hasOwn = ({}).hasOwnProperty, arr = [], pop = arr.pop, pushNative = arr.push,
            push = arr.push, slice = arr.slice,
            ...
            if (!seed) {
                ...
                if (documentIsHTML) {
                    ...
                    if (support.qsa && !nonnativeSelectorCache[selector + ' '] &&
                        ...
                        // In such cases, we work around the behavior by prefixing every selector in
                        // the list with an ID selector referencing the scope context. The technique
                        // has to be used as well when a leading combinator is used as such
                        // selectors are not recognized by querySelectorAll. Thanks to Andrew Dupont
                        // for this technique.
                        if (nodeType === 1 &&
        ...
        setDocument = Sizzle.setDocument = function(node) {
            ...
            // #13936) Support: IE 11+, Edge 17 - 18+ IE/Edge sometimes throw a "Permission denied"
            // error when strict-comparing two documents; shallow comparisons work.
            // eslint-disable-next-line eqeqeq
                        ...
                        while ((elem = elem[dir])) {
                            if (elem.nodeType === 1 || checkNonElements) {
                                ...
                                if (skip && skip === elem.nodeName.toLowerCase()) {
                                    ...
                                    (oldCache = uniqueCache[key]) && oldCache[0] === dirruns &&
                                    oldCache[1] === doneName) {
                    ...
                    return jQuery
                        .Deferred(function(newDefer) {
                            jQuery.each(tuples, function(_i, tuple) {
                                // Map tuples (progress, done, fail) to arguments (done, fail,
                                // progress)
                                var fn = isFunction(fns[tuple[4]]) && fns[tuple[4]];

                                // deferred.progress(function() { bind to newDefer or
                                // newDefer.notify }) deferred.done(function() { bind to newDefer or
                                // newDefer.resolve }) deferred.fail(function() { bind to newDefer
                                // or newDefer.reject })
                    ...
                    return jQuery
                        .Deferred(function(newDefer) {
                            ...
                            tuples[0][3].add(resolve(
                                0, newDefer, isFunction(onProgress) ? onProgress : Identity,
                                newDefer.notifyWith));
...
function adjustCSS(elem, prop, valueParts, tween) {
    ...
    if (valueParts) {
        ...
        adjusted =
            valueParts[1] ? initialInUnit + (valueParts[1] + 1) * valueParts[2] : +valueParts[2];
        if (tween) {
            tween.unit = unit;
...
jQuery.event = {
    ...
    add: function(elem, types, handler, data, selector) {
        var handleObjIn, eventHandle, tmp, events, t, handleObj, special, handlers, type,
            namespaces, origType, elemData = dataPriv.get(elem);
    ...
    handlers: function(event, handlers) {
        var i, handleObj, sel, matchedHandlers, matchedSelectors,
            handlerQueue = [], delegateCount = handlers.delegateCount, cur = event.target;
    ...
    if (!isBorderBox && computedVal >= 0) {
        ...
        delta += Math.max(
                     ...
                     Math.ceil(
                         ...
                         // content-box scroll gutter Use an explicit zero to avoid NaN (gh-3964)
                         )) ||
            0;
    }
...
function getWidthOrHeight(elem, dimension, extra) {
    ...
    return (val +
            boxModelAdjustment(
                elem, dimension, extra || (isBorderBox ? 'border' : 'content'), valueIsBorderBox,
                styles,
    ...
    jQuery.cssHooks[dimension] = {
        ...
        set: function(elem, value, extra) {
            var matches,
                ...
                subtract =
                    extra ? boxModelAdjustment(elem, dimension, extra, isBorderBox, styles) : 0;
        ...
        return {
            send: function(headers, complete) {
                ...
                callback = function(type) {
                    return function() {
                        if (callback) {
                            callback = errorCallback = xhr.onload = xhr.onerror = xhr.onabort =
