...
RaycastVehicle::RaycastVehicle(const btVehicleTuning& tuning,
                               btRigidBody* chassis,
...
//
btWheelInfo& RaycastVehicle::addWheel(
...
void RaycastVehicle::updateWheelTransform(int wheelIndex,
    ...
    btMatrix3x3 basis2(right[0], fwd[0], up[0], right[1], fwd[1], up[1],
                       right[2], fwd[2], up[2]);
...
void RaycastVehicle::updateWheelTransformsWS(btWheelInfo& wheel,
                                             bool interpolatedTransform) {
...
btScalar RaycastVehicle::rayCast(btWheelInfo& wheel) {
    updateWheelTransformsWS(wheel, false);
...
void RaycastVehicle::updateVehicle(btScalar step) {
    ...
    for (int i = 0; i < m_wheelInfo.size(); i++) {
        ...
        if (wheel.m_raycastInfo.m_isInContact) {
            ...
            if (i == 0 || i == 3) wheel.m_rotation -= wheel.m_deltaRotation;
            if (i == 1 || i == 2) wheel.m_rotation += wheel.m_deltaRotation;
...
btScalar RaycastVehicle::getSteeringValue(int wheel) const {
...
const btWheelInfo& RaycastVehicle::getWheelInfo(int index) const {
    btAssert((index >= 0) && (index < getNumWheels()));

    return m_wheelInfo[index];
}

btWheelInfo& RaycastVehicle::getWheelInfo(int index) {
    btAssert((index >= 0) && (index < getNumWheels()));
...
struct btWheelContactPoint {
    ...
    btWheelContactPoint(btRigidBody* body0, btRigidBody* body1,
        ...
        btScalar denom0 = body0->computeImpulseDenominator(
            frictionPosWorld, frictionDirectionWorld);
        btScalar denom1 = body1->computeImpulseDenominator(
            frictionPosWorld, frictionDirectionWorld);
...
void RaycastVehicle::updateFriction(btScalar timeStep) {
    ...
    for (int wheel = 0; wheel < getNumWheels(); wheel++) {
        ...
        if (m_sideImpulse[wheel] != btScalar(0.)) {
            class btRigidBody* groundObject =
                (class btRigidBody*)m_wheelInfo[wheel]
