...
void Robot::initPhysics(const bool& blue) {
    ...
    for (int i = 0; i <= numPoints; i++) {
        ...
        btVector3* pt =
            new btVector3(Sim_Robot_Radius * sin(angle), Sim_Robot_Height / 2.f,
                          Sim_Robot_Radius * cos(angle));
        btVector3* pb = new btVector3(Sim_Robot_Radius * sin(angle),
                                      -Sim_Robot_Height / 2.f,
                                      Sim_Robot_Radius * cos(angle));
    ...
    if (blue) *color = btVector3(0.f, 0.f, 1.f);
    ...
    btVector3 robotCenter =
        btVector3(0, Sim_Robot_Height / 2.f - connectionHeight, 0);
    _startTransform.setOrigin(robotCenter + _startTransform.getOrigin());

    // Create robot chassis rigid body
    _robotChassis =
        _simEngine->localCreateRobot(robotWeight, _startTransform, convexShape);

    // TODO: init robot ball controller
    _controller = new RobotBallController(this);
    _controller->initPhysics();

    ((RobotMotionState*)_robotChassis->getMotionState())->m_controller =
        _controller;
    ...
    {
        _vehicleRayCaster =
            new btDefaultVehicleRaycaster(_simEngine->dynamicsWorld());
        _robotVehicle =
            new RaycastVehicle(_tuning, _robotChassis, _vehicleRayCaster);
        ...
        // Position omni-wheels // Chassis orientation corrected for
        // automatically in addWheel()
        btVector3 connectPoint = btVector3(
            0, connectionHeight, Sim_Robot_Radius - Sim_Wheel_Width / 2.f);
        btVector3 forward = btVector3(0, 0, 1.0);
        // FL
        btVector3 connectionPointCS0 =
            connectPoint.rotate(btVector3(0, 1, 0), M_PI / 4.f * 1);
        wheelAxleCS = forward.rotate(btVector3(0, 1, 0), M_PI / 4.f * 1);
        _robotVehicle->addWheel(connectionPointCS0, wheelDirectionCS0,
                                wheelAxleCS, suspensionRestLength,
                                Sim_Wheel_Radius, _tuning, isFrontWheel);
        // FR
        connectionPointCS0 =
            connectPoint.rotate(btVector3(0, 1, 0), M_PI / 4.f * -1);
        wheelAxleCS = forward.rotate(btVector3(0, 1, 0), M_PI / 4.f * -1);
        ...
        // BR
        connectionPointCS0 =
            connectPoint.rotate(btVector3(0, 1, 0), M_PI / 4.f * 5);
        ...
        // BL
        connectionPointCS0 =
            connectPoint.rotate(btVector3(0, 1, 0), M_PI / 4.f * 3);
    ...
    _robotChassis->getBroadphaseHandle()->m_collisionFilterMask ^=
...
void Robot::position(float x, float y) {
    ...
    trans.setOrigin(
        btVector3(y * scaling, trans.getOrigin().y(),
                  x * scaling));  // map field coord -> engine WS; x->z, y->x;
...
Geometry2d::Point Robot::getVelFS() const {
    return Geometry2d::Point(_robotChassis->getLinearVelocity().z() / scaling,
                             _robotChassis->getLinearVelocity().x() / scaling);
}
...
Geometry2d::Point Robot::getTargetVelFS() const {
    return Geometry2d::Point(_targetVel.z() / scaling,
                             _targetVel.x() / scaling);
}

float Robot::getTargetAngVelFS() const { return _targetRot; }
...
void Robot::applyEngineForces(float deltaTime) {
    if (_targetVel.length() < SIMD_EPSILON && _targetRot == 0) {
        ...
        // basis vectors for robot - directions of driving two opposite wheels
        // "forward", i.e. towards z in CS
        // f right as in northeast, f left as in northwest
        btVector3 forwardRightCS =
            btVector3(-1, 0, 1)
                .normalize();  // positive x is left b/c y is up, very tricky
        btVector3 forwardLeftCS = btVector3(1, 0, 1).normalize();

        btVector3 robotVel = _robotChassis->getLinearVelocity();
        robotVel.setY(0);

        robotVel = robotVel.rotate(
            worldRot.getAxis(), -worldRot.getAngle());  // undo world rotation
        ...
        float wheelMultiplier = (Sim_Robot_Radius - (Sim_Wheel_Width / 2.f)) /
                                Sim_Wheel_Radius;  // move to actual wheel loc
        ...
        for (int i = 0; i < 4; i++) {
            _engineForce[i] +=
                forceRot / (4.f *
                            _robotChassis->getInvInertiaDiagLocal()
                                .y());  // cancel out inertia factors later
        }
...
void Robot::renderWheels(GL_ShapeDrawer* shapeDrawer,
...
void Robot::resetScene() {
    setEngineForce(0);
    _robotChassis->setCenterOfMassTransform(
        ((btDefaultMotionState*)_robotChassis->getMotionState())
            ->m_startWorldTrans);
    _robotChassis->setLinearVelocity(btVector3(0, 0, 0));
    _robotChassis->setAngularVelocity(btVector3(0, 0, 0));
    _simEngine->dynamicsWorld()
        ->getBroadphase()
        ->getOverlappingPairCache()
        ->cleanProxyFromPairs(_robotChassis->getBroadphaseHandle(),
                              _simEngine->dynamicsWorld()->getDispatcher());
    if (_robotVehicle) {
        _robotVehicle->resetSuspension();
        for (int i = 0; i < _robotVehicle->getNumWheels(); i++) {
            // synchronize the wheels with the (interpolated) chassis
