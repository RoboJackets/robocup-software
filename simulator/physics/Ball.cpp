...
void Ball::velocity(float x, float y) {
    if (_ball) _ball->setLinearVelocity(btVector3(y, 0, x) * scaling);
}
...
void Ball::initPhysics() {
    ...
    _ball =
        _simEngine->localCreateRigidBody(Sim_Ball_Mass, ballTr, _sphereShape);
    ...
    _ball->getBroadphaseHandle()->m_collisionFilterGroup |=
...
void Ball::resetScene() {
    if (_ball) {
        _ball->setCenterOfMassTransform(
            ((btDefaultMotionState*)_ball->getMotionState())
                ->m_startWorldTrans);
