...
struct btDebugCastResult : public btConvexCast::CastResult {
    ...
    virtual void DebugDraw(btScalar fraction) {
        ...
        btTransformUtil::integrateTransform(m_fromTrans, m_linVel, m_angVel,
                                            fraction, hitTrans);
        hitTrans.getOpenGLMatrix(m);
        if (m_shapeDrawer)
            m_shapeDrawer->drawOpenGL(m, m_shape, btVector3(1, 0, 0),
                                      btIDebugDraw::DBG_NoDebug, worldBoundsMin,
                                      worldBoundsMax);
    }
};
