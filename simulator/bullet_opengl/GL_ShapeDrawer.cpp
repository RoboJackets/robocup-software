...
public:
    ...
    virtual void processTriangle(btVector3* triangle, int partId,
        ...
        if (m_wireframe) {
            glBegin(GL_LINES);
            glColor3f(1, 0, 0);
            glVertex3d(triangle[0].getX(), triangle[0].getY(),
                       triangle[0].getZ());
            glVertex3d(triangle[1].getX(), triangle[1].getY(),
                       triangle[1].getZ());
            glColor3f(0, 1, 0);
            glVertex3d(triangle[2].getX(), triangle[2].getY(),
                       triangle[2].getZ());
            glVertex3d(triangle[1].getX(), triangle[1].getY(),
                       triangle[1].getZ());
            glColor3f(0, 0, 1);
        ...
        if (!(debugMode & btIDebugDraw::DBG_DrawWireframe)) {
            ...
            switch (shapetype) {
                case SPHERE_SHAPE_PROXYTYPE: {
                    ...
                    float radius = sphereShape->getMargin();  // radius doesn't
                                                              // include the
                                                              // margin, so draw
                                                              // with margin
			...
			case CONE_SHAPE_PROXYTYPE:
                ...
                /*
                   ...
                   static_cast<const btCylinderShape*>(shape);
                                    ...
                                    float halfHeight =
                   cylinder->getHalfExtentsWithMargin()[upAxis];
                ...
                */
                ...
                case MULTI_SPHERE_SHAPE_PROXYTYPE: {
                    ...
                    for (int i = multiSphereShape->getSphereCount() - 1; i >= 0;
                         i--) {
                        ...
                        drawOpenGL(childMat, &sc, color, debugMode,
                                   worldBoundsMin, worldBoundsMax);
                    }
        ...
        if (shape->isConvex()) {
            ...
            for (int i = 0; i < sc->m_edges.size(); ++i) {
                ...
                if ((d * btDot(sc->m_edges[i].n[1], extrusion)) < 0) {
                    const int q = d < 0 ? 1 : 0;
                    const btVector3& a =
                        hull->getVertexPointer()[sc->m_edges[i].v[q]];
                    const btVector3& b =
