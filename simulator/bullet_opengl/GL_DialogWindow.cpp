...
GL_DialogWindow::GL_DialogWindow(int horPos, int vertPos, int dialogWidth,
                                 int dialogHeight,
...
static void drawLine(int _X0, int _Y0, int _X1, int _Y1, unsigned int _Color0,
                     unsigned int _Color1) {
    const GLfloat dx = +0.5f;
    const GLfloat dy = -0.5f;

    GLfloat vVertices[] = {(GLfloat)_X0 + dx, (GLfloat)_Y0 + dy,
                           (GLfloat)_X1 + dx, (GLfloat)_Y1 + dy};
...
#ifdef USE_ARRAYS
    glColor4ub(GLubyte(_Color0 >> 16), GLubyte(_Color0 >> 8), GLubyte(_Color0),
               GLubyte(_Color0 >> 24));
    ...
    glColor4ub(GLubyte(_Color1 >> 16), GLubyte(_Color1 >> 8), GLubyte(_Color1),
               GLubyte(_Color1 >> 24));
...
static void drawRect(int horStart, int vertStart, int horEnd, int vertEnd,
                     unsigned int argbColor00, unsigned int argbColor10,
                     unsigned int argbColor01, unsigned int argbColor11) {
    float dx = 0;
    float dy = 0;
    glDisable(GL_TEXTURE_2D);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
#ifdef USE_ARRAYS
    GLfloat verts[] = {0.0f, 1.0f,  0.0f, -1.0f, -1.0f, 0.0f,
                       1.0f, -1.0f, 0.0f, 0.f,   0.f,   0.f};

    glColor4ub(GLubyte(argbColor00 >> 16), GLubyte(argbColor00 >> 8),
               GLubyte(argbColor00), GLubyte(argbColor00 >> 24));
    ...
    glColor4ub(GLubyte(argbColor10 >> 16), GLubyte(argbColor10 >> 8),
               GLubyte(argbColor10), GLubyte(argbColor10 >> 24));
    glVertex2f((GLfloat)horEnd + dx, (GLfloat)vertStart + dy);
    glColor4ub(GLubyte(argbColor11 >> 16), GLubyte(argbColor11 >> 8),
               GLubyte(argbColor11), GLubyte(argbColor11 >> 24));
    glVertex2f((GLfloat)horEnd + dx, (GLfloat)vertEnd + dy);
    glColor4ub(GLubyte(argbColor01 >> 16), GLubyte(argbColor01 >> 8),
               GLubyte(argbColor01), GLubyte(argbColor01 >> 24));
...
void GL_DialogWindow::draw(btScalar deltaTime) {
    ...
    drawRect(m_dialogHorPos, m_dialogVertPos,
             m_dialogHorPos + m_dialogWidth - 1, m_dialogVertPos + titleHeight,
             argbColor, argbColor, argbColor, argbColor);
    // const unsigned int COL0 = 0x50ffffff;
    const unsigned int COL0 = 0xffffffff;
    const unsigned int COL1 = 0xff1f1f1f;

    drawRect(m_dialogHorPos, m_dialogVertPos,
             m_dialogHorPos + m_dialogWidth - 1, m_dialogVertPos + 1, COL0,
             COL0, COL1, COL1);

    argbColor = 0x864f4f4f;
    drawRect(m_dialogHorPos + 1, m_dialogVertPos + titleHeight,
             m_dialogHorPos + m_dialogWidth - 1,
             m_dialogVertPos + m_dialogHeight, argbColor, argbColor, argbColor,
             argbColor);

    int y = m_dialogVertPos + charHeight + 1;
    glLineWidth(3);
    drawLine(m_dialogHorPos, y, m_dialogHorPos + m_dialogWidth - 1, y,
             0x80afafaf, 0x80afafaf);

    unsigned int clight = 0x5FFFFFFF;  // bar contour
    drawLine(m_dialogHorPos, m_dialogVertPos, m_dialogHorPos,
             m_dialogVertPos + m_dialogHeight, clight, clight);
    drawLine(m_dialogHorPos, m_dialogVertPos, m_dialogHorPos + m_dialogWidth,
             m_dialogVertPos, clight, clight);
    drawLine(m_dialogHorPos + m_dialogWidth, m_dialogVertPos,
             m_dialogHorPos + m_dialogWidth, m_dialogVertPos + m_dialogHeight,
             clight, clight);
    drawLine(m_dialogHorPos, m_dialogVertPos + m_dialogHeight,
             m_dialogHorPos + m_dialogWidth, m_dialogVertPos + m_dialogHeight,
             clight, clight);
    int dshad = 3;  // bar shadows

    unsigned int cshad = (((0x40000000 >> 24) / 2) << 24) & 0xFF000000;
    drawRect(m_dialogHorPos, m_dialogVertPos + m_dialogHeight,
             m_dialogHorPos + dshad, m_dialogVertPos + m_dialogHeight + dshad,
             0, cshad, 0, 0);
    drawRect(m_dialogHorPos + dshad + 1, m_dialogVertPos + m_dialogHeight,
             m_dialogHorPos + m_dialogWidth - 1,
             m_dialogVertPos + m_dialogHeight + dshad, cshad, cshad, 0, 0);
    drawRect(m_dialogHorPos + m_dialogWidth, m_dialogVertPos + m_dialogHeight,
             m_dialogHorPos + m_dialogWidth + dshad,
             m_dialogVertPos + m_dialogHeight + dshad, cshad, 0, 0, 0);
    drawRect(m_dialogHorPos + m_dialogWidth, m_dialogVertPos,
             m_dialogHorPos + m_dialogWidth + dshad, m_dialogVertPos + dshad, 0,
             0, cshad, 0);
    drawRect(m_dialogHorPos + m_dialogWidth, m_dialogVertPos + dshad + 1,
             m_dialogHorPos + m_dialogWidth + dshad,
             m_dialogVertPos + m_dialogHeight - 1, cshad, 0, cshad, 0);
    ...
    GLDebugDrawString(m_dialogHorPos + m_dialogWidth / 2 -
                      ...
                      m_dialogVertPos + yInc, m_dialogTitle);
    curVertPos += 20;
...
void GL_TextControl::draw(int& parentHorPos, int& parentVertPos,
                          btScalar deltaTime) {
...
void GL_ToggleControl::draw(int& parentHorPos2, int& parentVertPos2,
                            btScalar deltaTime) {
    ...
    drawRect(parentHorPos, parentVertPos, parentHorPos + 16, parentVertPos + 16,
             grey, grey, grey, grey);

    int borderSize = 2;
    unsigned int white = 0xffefefef;
    drawRect(parentHorPos + borderSize, parentVertPos + borderSize,
             parentHorPos + 16 - borderSize, parentVertPos + 16 - borderSize,
    ...
    if (m_active) {
        ...
        drawRect(parentHorPos + borderSize, parentVertPos + borderSize,
                 ...
                 parentVertPos + 16 - borderSize, black, black, black, black);
    }

    btVector3 rgb(1, 1, 1);

    GLDebugDrawStringInternal(parentHorPos2, parentVertPos + 16, m_toggleText,
                              rgb);
    parentVertPos2 += 20;
}

void GL_SliderControl::draw(int& parentHorPos2, int& parentVertPos2,
                            btScalar deltaTime) {
    ...
    int sliderPosE =
        parentHorPos2 + m_parentWindow->getDialogWidth() - 40 - borderSize;
