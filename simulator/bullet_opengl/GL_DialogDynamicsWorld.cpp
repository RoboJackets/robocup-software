...
GL_DialogDynamicsWorld::GL_DialogDynamicsWorld() {
    ...
    btConvex2dConvex2dAlgorithm::CreateFunc* convexAlgo2d =
        new btConvex2dConvex2dAlgorithm::CreateFunc(m_simplexSolver,
                                                    m_pdSolver);
    ...
    m_dispatcher->registerCollisionCreateFunc(
        BOX_2D_SHAPE_PROXYTYPE, BOX_2D_SHAPE_PROXYTYPE,
...
void GL_DialogDynamicsWorld::setScreenSize(int width, int height) {
        ...
        if (body) {
            ...
            btVector3 newPos =
                ...
                btVector3(btScalar(m_screenWidth / 2.),
                          btScalar(m_screenHeight / 2), btScalar(0)) -
                btVector3(btScalar(width / 2.), btScalar(height / 2.),
                          btScalar(0));
    ...
    if (width && height) {
        if (m_upperBorder) {
            ...
            tr.setIdentity();
            tr.setOrigin(
                btVector3(btScalar(0), btScalar(-height / 2.), btScalar(0.)));
            m_upperBorder->setWorldTransform(tr);
            m_dynamicsWorld->addCollisionObject(m_upperBorder);
        }
        ...
        if (m_leftBorder) {
            ...
            tr.setIdentity();
            tr.setOrigin(
                btVector3(btScalar(-width / 2.), btScalar(0), btScalar(0)));
            m_leftBorder->setWorldTransform(tr);
            m_dynamicsWorld->addCollisionObject(m_leftBorder);
        }
        if (m_rightBorder) {
...
GL_DialogWindow* GL_DialogDynamicsWorld::createDialog(int horPos, int vertPos,
                                                      int dialogWidth,
                                                      int dialogHeight,
                                                      const char* dialogTitle) {
    btBox2dShape* boxShape = new btBox2dShape(
        btVector3(dialogWidth / 2.f, dialogHeight / 2.f, 0.4f));
    btScalar mass = 100.f;
    btVector3 localInertia;
    boxShape->calculateLocalInertia(mass, localInertia);
    btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, nullptr, boxShape,
                                                    localInertia);
    ...
    GL_DialogWindow* dialogWindow = new GL_DialogWindow(
        horPos, vertPos, dialogWidth, dialogHeight, body, dialogTitle);
...
GL_SliderControl* GL_DialogDynamicsWorld::createSlider(
    GL_DialogWindow* dialog, const char* sliderText, btScalar initialFraction) {
    ...
    int sliderY = dialog->getDialogVertPos() - m_screenHeight / 2 +
                  dialog->getDialogHeight() / 2 + dialog->getNumControls() * 20;
    trans.setOrigin(
        btVector3((btScalar)sliderX, (btScalar)sliderY, (btScalar)-0.2f));
    ...
    m_dynamicsWorld->addRigidBody(body);
    body->setCollisionFlags(body->getFlags() |
    ...
    int offsY =
        -dialog->getDialogHeight() / 2 + dialog->getNumControls() * 20 + 36;
    btVector3 offset(
        btVector3((btScalar)offsX, (btScalar)offsY, (btScalar)0.2f));
    ...
    btScalar actualLimit =
        lowerLimit + initialFraction * (upperLimit - lowerLimit);
    ...
    btSliderConstraint* sliderConstraint = new btSliderConstraint(
        *dialogBody, *body, frameInA, frameInB, true);  // useFrameA);
    ...
    GL_SliderControl* slider = new GL_SliderControl(
        sliderText, body, dialog, lowerLimit, upperLimit, sliderConstraint);
...
GL_ToggleControl* GL_DialogDynamicsWorld::createToggle(GL_DialogWindow* dialog,
    ...
    int toggleY = dialog->getDialogVertPos() - m_screenHeight / 2 +
                  dialog->getDialogHeight() / 2 + dialog->getNumControls() * 20;
    trans.setOrigin(
        btVector3((btScalar)toggleX, (btScalar)toggleY, (btScalar)-0.2f));
    ...
    btTransform frameInA;
    frameInA.setIdentity();
    btVector3 offset(btVector3(+dialog->getDialogWidth() / 2.f - 32.f,
                               -dialog->getDialogHeight() / 2.f +
                                   dialog->getNumControls() * 20.f + 36.f,
                               0.2f));
    frameInA.setOrigin(offset);
    ...
    btGeneric6DofConstraint* constraint = new btGeneric6DofConstraint(
        *dialogBody, *body, frameInA, frameInB, useFrameA);
    m_dynamicsWorld->addConstraint(constraint, true);
            ...
            if (state == 0) {
                    ...
                    if (rayCallback.hasHit()) {
                        ...
                        if (body) {
                            ...
                            if (body->getUserPointer()) {
                                ...
                                GL_DialogControl* ctrl =
                                    (GL_DialogControl*)body->getUserPointer();

                                switch (ctrl->getType()) {
                                    case GL_TOGGLE_CONTROL: {
                                        GL_ToggleControl* toggle =
                                            (GL_ToggleControl*)ctrl;
                                    ...
                                    case GL_SLIDER_CONTROL: {
                                        GL_SliderControl* slider =
                                            (GL_SliderControl*)ctrl;
                                        btTypedConstraint* constraint =
                                            slider->getConstraint();
                ...
                if (m_pickConstraint && m_dynamicsWorld) {
                    ...
                    if (pickedBody->getUserPointer()) {
                        /// deal with controls in a special way
                        GL_DialogControl* ctrl =
                            (GL_DialogControl*)pickedBody->getUserPointer();
                        if (ctrl->getType() == GL_SLIDER_CONTROL) {
                            GL_SliderControl* sliderControl =
                                (GL_SliderControl*)ctrl;

                            btSliderConstraint* slider = nullptr;
                            btTypedConstraint* constraint =
                                sliderControl->getConstraint();
                            if (constraint->getConstraintType() ==
                                SLIDER_CONSTRAINT_TYPE) {
