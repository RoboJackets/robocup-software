...
void DemoApplication::zoomIn() {
    ...
    updateCamera();
    if (m_cameraDistance < btScalar(0.1)) m_cameraDistance = btScalar(0.1);
}
...
void DemoApplication::pickObject(const btVector3& pickPos,
...
void DemoApplication::showProfileInfo(int& xOffset, int& yStart, int yIncr) {
...

    {
        ...
        for (int i = 0; !m_profileIterator->Is_Done();
             m_profileIterator->Next()) {
            double current_total_time =
                m_profileIterator->Get_Current_Total_Time();
            ...
            sprintf(blockTime,
                    "%d -- %s (%.2f %%) :: %.3f ms / frame (%d calls)", ++i,
                    m_profileIterator->Get_Current_Name(), fraction,
        ...
        sprintf(
            blockTime, "%s (%.3f %%) :: %.3f ms", "Unaccounted",
                ...
                for (iter = btProfiler::mProfileBlocks.begin();
                    ...
                    sprintf(
                        ...
                        btProfiler::getBlockTime(
                            (*iter).first,
                            btProfiler::
                                BLOCK_CYCLE_SECONDS));  // BLOCK_TOTAL_PERCENT));
